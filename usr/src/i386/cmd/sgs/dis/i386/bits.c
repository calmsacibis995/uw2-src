	...
	for (loc = shdr->sh_addr; ((loc-shdr->sh_addr) < shdr->sh_size) && (errlev < MAXERRS); printline()){
		...
		*/
		...
		case IMlw:
			...
			/* Check for opcode 83 before passing in a 0 or 1 (3rd    */
			/* param) through to convert to tell it whether to print  */
			/* a sign extended immediate byte.		          */ 
			if (opcode1 == 8 && opcode2 == 3)
				imm_data(OPSIZE(data16,opcode2 == 1), 0, 1);
			else
				imm_data(OPSIZE(data16,opcode2 == 1), 0, 0);
