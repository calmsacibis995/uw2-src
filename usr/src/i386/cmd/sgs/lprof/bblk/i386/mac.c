...
#ident	"@(#)lprof:bblk/i386/mac.c	1.4.1.1"
...
#include <pfmt.h>	/* standard I/O header file */
#include <unistd.h>	/* standard I/O header file */
...
char *text;
{
	...
	if (text == NULL) {
		pfmt(stderr,MM_ERROR,":1226:CAinstr(): %s\n", gettxt(":1221",err_msg[SR107]));
		exit(FATAL);
	}
...
unsigned long int *stlnary;
{
	...
	if (bct == ZERO) {
		pfmt(stderr,MM_ERROR,":1227:CAsize(): %s\n", gettxt(":1219",err_msg[SR105]));
		exit(FATAL);
	}

	/* prints alignment */
	if (fprintf(stdout,_ALIGN) <= PNT_FAIL) {
		pfmt(stderr,MM_ERROR,":1227:CAsize(): %s\n", gettxt(":1223",err_msg[SR109]));
		exit(FATAL);
	}
