...
#ident	"@(#)r5server:dix/property.c	1.2"
    ...
    ClientPtr client;
{
    ...
    for (i = 0; i < (int)stuff->nAtoms; i++)
    {
        ...
        for (j = i + 1; j < (int)stuff->nAtoms; j++)
            if (atoms[j] == atoms[i])
            {
                DEALLOCATE_LOCAL(props);
                return BadMatch;
...
found: 
    ...
    if ( (stuff->nAtoms != 0) &&
	(abs(delta) % ((unsigned) stuff->nAtoms)) != 0 ) 
    {
	while (delta < 0)                  /* faster if abs value is small */
    	...
    	for (i = 0; i < (int)stuff->nAtoms; i++)
 	{
	    ...
	    DeliverEvents(pWin, &event, 1, (WindowPtr)NULL);
            ...
            props[i]->propertyName = atoms[(i + delta) % ((unsigned) stuff->nAtoms)];
	}
    }
    DEALLOCATE_LOCAL(props);
    return Success;
