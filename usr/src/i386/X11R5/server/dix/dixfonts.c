...
#ident	"@(#)r5server:dix/dixfonts.c	1.9"
#include <stdio.h>
...
/* this pointer was not being initialized */
static unsigned char *font_path_string = (unsigned char *) 0;
    ...
    XID          fid;
{
	...
	FreeFontDevPrivates(pfont); /* added */
    ...
    LFclosurePtr c;
{
	...
	if (err == Suspended) {
	    if (!c->slept) {
		c->slept = TRUE;
		ClientSleep(client, doListFonts, (unsigned char *)c);
	    }
	    return TRUE;
	}
	if (err != Successful)
	...
	if (!c->current.list_started)
 	{
		...
		if (!c->slept)
 		{
		    ClientSleep(client, doListFontsWithInfo,
				(unsigned char *)c);
		    c->slept = TRUE;
		}
		return TRUE;
    ...
    int         n;
{
    ...
    xfree((char *) list);
    xfree(font_path_string); /* added */
    font_path_string = (unsigned char *) 0; /* added */
}
