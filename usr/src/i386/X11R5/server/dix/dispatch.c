...
#pragma ident	"@(#)r5server:dix/dispatch.c	1.20"
...
#ifdef __STDC__
static void SwapFourBytes (unsigned char *src,
			   unsigned char *dest,
			   long srclen,
			   long srcinc,
			   long destinc,
...
Dispatch()
{
    ...
    static unsigned long myGeneration = 0;

    /* For built-in, Dispatch() gets called and returns more than "once" so
     * don't want to ALLOC and DEALLOC clientReady each time
     */
#ifdef BUILTIN
    int		clientReady[128];	/* array of request ready clients */
#else
    ...
    /* Variables below should only be (re)initialized per serverGeneration */
...
#if !defined(XTRAP) || defined(NO_NEW_XTRAP)	/* new XTRAP */
...
#ifdef BUILTIN
	/* Consider dispatching to internal (built-in) clients. */
	if (ARE_BUILTINS() &&
	    (((BIGlobal.dispatch_tv_sec == 0L) &&	/* implies */
    ...
    register ClientPtr client;
{
        ...
        if (!(pSrc = LOOKUP_DRAWABLE(stuff->srcDrawable, client)))
	{
	    client->errorValue = stuff->srcDrawable;
            return(BadMatch); /* was BadDrawable */
	}
    ...
    register ClientPtr client;
{
    ...
    {
        if ((stuff->depth != 1) ||
	    ((int)stuff->leftPad >= (int)screenInfo.bitmapScanlinePad))
            return BadMatch;
        length = PixmapBytePad((int)(stuff->width + stuff->leftPad), 1);
    }
    else if (stuff->format == XYPixmap)
    {
        if ((pDraw->depth != stuff->depth) || 
	    (stuff->leftPad >= (unsigned)screenInfo.bitmapScanlinePad))
            return BadMatch;
        length = PixmapBytePad((int)(stuff->width + stuff->leftPad), 1);
	length *= stuff->depth;
    }
    else if (stuff->format == ZPixmap)
    {
        if ((pDraw->depth != stuff->depth) || (stuff->leftPad != 0))
            return BadMatch;
        length = PixmapBytePad((int)stuff->width, stuff->depth);
    }
    else
    {
...
/* Hack for Xlib XPutImage bug */
    if (screenInfo.bitmapBitOrder != screenInfo.imageByteOrder)
    {
        ...
        unsigned int bytes_per_line =
            PixmapBytePad((int)(stuff->width + stuff->leftPad), 1);
...
#if 0
	/* this must be wrong, it never sets src */
	/* it probably worked for small mono images, but not color */
...
#else /* this is probably more correct */
	for (src = (unsigned char *)&stuff[1], dest = tmpbits, i = 0;
	     i < stuff->depth;
             src += bytes_per_plane, dest += bytes_per_plane)
            SwapFourBytes (src, dest, bytes_per_src, bytes_per_line,
                       bytes_per_line, stuff->height, MSBFirst);
#endif
    ...
    register ClientPtr	client;
{
    ...
    {
	widthBytesLine = PixmapBytePad((int)stuff->width, pDraw->depth);
	length = widthBytesLine * height;
    }
    else 
    {
	widthBytesLine = PixmapBytePad((int)stuff->width, 1);
...
#define FontShiftSize 5
	...
	else	/* print a string */
	{
	    ...
	    (void) LoadGlyphs(client, pGC->font, *pElt, itemSize,
			      (unsigned char *)(pElt + TextEltHeader));
    ...
    register ClientPtr client;
{
	...
	if(OsLookupColor(pcmp->pScreen->myNum, (unsigned char *)&stuff[1],
			 (unsigned long)stuff->nbytes,
    ...
    register ClientPtr client;
{
    ...
    n = PixmapBytePad((int)width, 1)*height;
    ...
    register ClientPtr client;
{
    ...
    if (total >= 4)
	return(BadLength);
    result = SetFontPath(client, stuff->nFonts,
			 (unsigned char *)&stuff[1], &error);
    if (error != -1)
    ...
    register ClientPtr client;
{
    ...
    if ((stuff->mode == DestroyAll) || /* was 'AllTemporary' */
