...
#ident	"@(#)r5server:dix/dixutils.c	1.3"
...
KeyClassPtr keyc;
{
	...
	/*
	 * location maxKeyPerModifier is equivent to [1][0] into modifierkeymap
	 * which is the row containing th 'lock' modifier keycodes. We'll
	 * scan through that line then using non-zero keycodes find the
	 * associated keysym. If it is XK_Shift_Mask then lock_meaning is	
	 * is set to that.
	 * selection logic is patterend after InitModMap in lib/X/XKeyBind.c
	 */

	keyc->lock_meaning = NoSymbol;
	for(i = keyc->maxKeysPerModifier; i < (2 * keyc->maxKeysPerModifier); i++) {
		mod_kc = keyc->modifierKeyMap[i];
		if(mod_kc) {
			mod_kc -= keyc->curKeySyms.minKeyCode;
			for(j=mod_kc*keyc->curKeySyms.mapWidth; j<(mod_kc + 1)*keyc->curKeySyms.mapWidth; j++) {
				if (keyc->curKeySyms.map[j] == XK_Caps_Lock) {
					keyc->lock_meaning = XK_Caps_Lock;	
					break;
				}
				else if (keyc->curKeySyms.map[j] == XK_Shift_Lock) {
