...
#ident	"@(#)r5server:dix/window.c	1.7"
    ...
    unsigned short width;
{
    ...
    pScreen = pWin->drawable.pScreen;
    pParent = pWin->parent;
    if (WasViewable && (int)width < (int)oldwidth)
	anyMarked = MarkOverlappedWindows(pWin, pWin);
...
#if 1
    /* XXX:md - we need two boxes:
     *   localBox: (local coords) use with the WindowExtents functions
     *   box:      (global coords) use with the RectIn functions
     */
    BoxRec localBox;
...
#define localBox box
    ...
    localBox = box;
    if (pParent) {
	localBox.x1 -= pParent->drawable.x;
	localBox.y1 -= pParent->drawable.y;
	localBox.x2 -= pParent->drawable.x;
	localBox.y2 -= pParent->drawable.y;
    }
#endif
    switch (smode)
    {
	...
	else if (pSib)
	{
        ...
        else if (AnyWindowOverlapsMe(pWin, pHead, &localBox))
            return(pFirst);
        else
            return(pWin->nextSib);
      case BottomIf:
	...
	else if (pSib)
	{
        ...
        else if (IOverlapAnyWindow(pWin, &localBox))
            return NullWindow;
        else
            return(pWin->nextSib);
      case Opposite:
    ...
    register ClientPtr client;
{
    ...
    for (j=0; j<client->numSaved; j++)
    {
        ...
        if (pParent && (pWin->parent != pParent))
	{
	    /* was missing pParent->drawable.[xy] */
            ReparentWindow(pWin, pParent, pWin->drawable.x -
			   wBorderWidth (pWin) - pParent->drawable.x,
