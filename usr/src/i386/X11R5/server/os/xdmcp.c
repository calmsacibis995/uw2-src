...
static void get_manager_by_name(), get_xdmcp_sock();

static	void receive_packet(), send_packet();
static	void timeout(), restart();

static	void recv_willing_msg();
static	void recv_accept_msg(),	recv_decline_msg();
static	void recv_refuse_msg(),	recv_failed_msg();
    ...
    Bool    (*AddAuth)();
{
    ...
    for (i = 0; i < (int)AuthenticationNames.length - 1; i++)
    ...
    unsigned		length;
{
	...
	XdmcpReadARRAY8 (&buffer, &AcceptAuthorizationData))
    {
	    ...
	    AugmentSelf ((struct sockaddr *)&req_sockaddr, req_socklen);
