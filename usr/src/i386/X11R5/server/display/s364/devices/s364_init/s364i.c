...
#pragma ident	"@(#)s364:s364/devices/s364_init/s364i.c	1.3"

/***
 ***	NAME
 ***
 ***		s364i.c : ati pro turbo card init functions
 ***
 ***	SYNOPSIS
 ***
 ***		
 ...
 ***	Constants.
 ***/
#define DEFAULT_S364I_COMPATIBILITY_STRING_OPTION_SIZE	1024

#define DEFAULT_S364I_COMPATIBILITY_CHIPSET_NAME 		"S364I"

#define DEFAULT_S364I_COMPATIBILITY_IDENT_STRING			\
	"S3Vision864/S3Vision964 Board.\n"

#define DEFAULT_S364I_COMPATIBILITY_INFO_FORMAT			\
	"%s %s %dx%d %gx%g %d %dK"

#define DEFAULT_S364I_COMPATIBILITY_INFO_TO_VENDOR_LIB	\
 ...
 * PURPOSE
 *
 *		This is the first entry point called by the server(R5) on 
 * loading the display library to initialize the chipset. 
 *
 * RETURN VALUE
 *
				...
				SIScreenRec *si_screen_p)
{
...
#if (defined(__DEBUG__))
	extern void s364i_debug_control(boolean);
	
	s364i_debug_control(TRUE);
	if (s364i_debug)
	{
		(void) fprintf(debug_stream_p, 
			"(s364i) DM_InitFunction {\n"
	...
	(void) fprintf(stderr, S364I_STARTUP_MESSAGE);

	/*
	 * Allocate space for the screen state and tuck away the pointer.
	 */
	screen_state_p = 
		allocate_and_clear_memory(sizeof(struct s364_screen_state));
	 ...
	 */
	initialization_status = s364_initialize_display_library(
	...
	if (initialization_status)
	{
		...
		if (s364i_debug)
		{
			(void) fprintf(debug_stream_p, "(s364i) chipset init "
					"failed.\n");
		}
		...
		{
			(void) fprintf(stderr,
				S364I_UNABLE_TO_RESTORE_TEXTMODE_MESSAGE);
		}
#endif
		return (SI_FAIL);
	}
 ...
 * servers.
 */
extern SIFunctions s364i_compatibility_display_functions;
...
STATIC SIBool
s364i_compatibility_init(int vt_fd, SIConfigP old_cfg_p, SIInfoP	info_p,
	ScreenInterface **routines_pp)
{
	SIScreenRec *si_screen_p;	/* new structure to pass down */
	int video_ram_size;
	int vfreq;
	char model_name[DEFAULT_S364I_COMPATIBILITY_STRING_OPTION_SIZE];
	char *vfreq_p;
	float monitor_width, monitor_height;
	int disp_w, disp_h;
	int depth;
	SIConfigP config_p;
	char monitor_name[DEFAULT_S364I_COMPATIBILITY_STRING_OPTION_SIZE];
	...
	si_screen_p->funcsPtr = *routines_pp =
		&s364i_compatibility_display_functions;
	...
	config_p->chipset = DEFAULT_S364I_COMPATIBILITY_CHIPSET_NAME;
	
	config_p->IdentString =
		DEFAULT_S364I_COMPATIBILITY_IDENT_STRING;

	if (sscanf(config_p->info,
		DEFAULT_S364I_COMPATIBILITY_INFO_FORMAT,
		...
		&video_ram_size) != 8)
	{
		...
		(void) fprintf(stderr,
			S364I_COMPATIBILITY_BAD_INFO_STRING_MESSAGE,
			config_p->info);
		return (SI_FAIL);
	}
	...
	{
		(void) fprintf(stderr,
			   S364I_COMPATIBILITY_BAD_MONITOR_DIMENSIONS_MESSAGE,
	...
	{
		(void) fprintf(stderr,
			S364I_COMPATIBILITY_BAD_MONITOR_NAME_MESSAGE, monitor_name);
			return (SI_FAIL);
	}
	 ...
	 * For more details on setting virtual display, read README.S364
	 */
	if ((virtual_dimension_string_p = getenv("S364I_VIRTUAL_DISPLAY")) != NULL)
	{
		...
		if (sscanf(virtual_dimension_string_p,"%ldx%ld\n",&virt_w,&virt_h) != 2)
		{
			 ...
			 */
			(void) fprintf (stderr, 
			S364I_COMPATIBILITY_INVALID_VIRTUAL_DISPLAY_STRING_MESSAGE);
		}
		else 
		{
...
SIFunctions *DisplayFuncs = &s364i_compatibility_display_functions;
