...
#ident	"@(#)vga256:vga256/devices/etw32/cursor.c	1.2"
...
int etw32_curs_index;		/* index of currently-displayed cursor */
int etw32_curs_on = 0;		/* 1 if currently displayed */
int etw32_curs_x = 0;		/* x position on the screen of cursor's org */
int etw32_curs_y = 0;		/* y position on the screen of cursor's org */
int etw32_curs_fg[CURS_COUNT];	/* cursor foreground colors */
int etw32_curs_bg[CURS_COUNT];	/* cursor background color */
int etw32_curs_region;		/* board address of where cursors are stored */
unchar etw32_curs_savedata[CURS_REGION_SIZE];	/* save cursor(s) on restore */
...

/*
 *   etw32_curs_download(index, cp) : Read a cursor description into the
...
SICursorP	cp;
{
    ...
    was_on = etw32_curs_on;
    if (etw32_curs_on)
      etw32_curs_turnoff(index);

      /* set up new cursor information */
    etw32_curs_fg[index] = cp->SCfg;
    etw32_curs_bg[index] = cp->SCbg;
...
/*
 *   etw32_curs_turnon(index) : Turn on HW cursor
 *	Input: 
...
int index;
{
    ...
    if (!etw32_curs_on) {
	int curs_addr = (etw32_curs_region + index*CURS_SIZE) >> 2;
	etw32_curs_index = index;
	etw32_curs_on = 1;
...
/*
 *   etw32_curs_turnoff(index) : Turn off HW cursor (remove from display).
 *
 *	Input: 
...
/*
 *   etw32_curs_move(index, x, y) : Move HW cursor
 *
 *	Input: 
 *	   int	index  : Index of cursor (currently ignored)
...
/*
 *   etw32_curs_restore(index) : Return HW cursor to default state.
 */
SIBool
etw32_curs_restore()
{
    ...
    W_IMA_CTRL(R_IMA_CTRL() & ~IMACTRL_CSREN);
    /* save cursor data */
    memmove(MMU_AP2 + 8, etw32_curs_savedata, CURS_REGION_SIZE);
    for (i=0xE0; i<0xF0; ++i) {
	etw32_set_reg(CSR_PORT, i, saved_csr_regs[i]);
    }
...
/*
 *   etw32c13_curs_init() : Called at startup and after each VT flip.
...
etw32_curs_init()
{
	...
	++etw32_inited;
	for (i=0xE0; i<0xF8; ++i) {
	    saved_csr_regs[i] = etw32_get_reg(CSR_PORT, i);
	    fprintf(stderr, "%2x = %2x\n", i, etw32_get_reg(CSR_PORT, i));
	}
	fprintf(stderr, "MMU_BASE2 = %x\n", MMU_BASE2);
     } 
    /* restore cursor data */
    selectpage(etw32_curs_region);
    memmove(etw32_curs_savedata, MMU_AP2 + 8, CURS_REGION_SIZE);

    /* restore cursor registers */
    etw32_curs_move(0, etw32_disp_x, etw32_disp_y);
    if (etw32_curs_on) {
	etw32_curs_on = 0;
	etw32_curs_turnon(etw32_curs_index);
    }
    return SI_SUCCEED;
}
