...
#pragma ident	"@(#)vga256:vga256/devices/boca/boca_256.c	1.10"
...
/* Type 0, Tseng Labs BOCA VGA 1024x768 256 colors CRYSTALSCAN */
	/* sequencer */
	0x01, 0x01, 0x0f, 0x00, 0x0e,
	/* misc */
	0x27,
	/* CRTC */
	0x99, 0x7f, 0x80, 0x1d, 0x88, 0x17, 0x2f, 0xf5,
	0x00, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x08, 0x80, 0xff, 0x80, 0x60, 0xff, 0x30, 0xab, 0xff,

/* Type 1, Tseng Labs BOCA VGA 1024x768 256 colors, interlaced */
	/* sequencer */
	0x01, 0x01, 0x0f, 0x00, 0x0e,
	/* misc */
	0x27,
...
/* Type 2, Tseng Labs BOCA VGA 800x600 256 colors */
	/* sequencer */
	0x01, 0x01, 0x0f, 0x00, 0x0e,
	/* misc */
	0x2f,
...
/* Type 3, Tseng Labs BOCA VGA 800x600 256 colors alternate */
	/* sequencer */
	0x01, 0x01, 0x0f, 0x00, 0x0e,
	/* misc */
	0xeb,
...
/* Type 4, Tseng Labs BOCA VGA 800x600 256 colors CRYSTALSCAN monitor */
	/* sequencer */
	0x01, 0x01, 0x0f, 0x00, 0x0e,
	/* misc */
	0x6b,
...
/* Type 5, Tseng Labs BOCA VGA 800x600 256 colors for interlaced monitors */ 
	/* sequencer */
	0x01, 0x01, 0x0f, 0x00, 0x0e,
	/* misc */
	0x27,
...
/* Type 6, Tseng Labs BOCA VGA 640x480 256 colors */
	/* sequencer */
	0x01, 0x01, 0x0f, 0x00, 0x0e,
	/* misc */
	0xe3,
...
struct	_DispM mode_data[] = {	/* display info for support adapters */
 { BOCA_1024, "BOCA", "MULTISYNC", 1024, 768, &(inittab[BOCA_1024]) },
 { BOCA_1024i, "BOCA", "INTERLACED", 1024, 768, &(inittab[BOCA_1024i]) },
 { BOCA_800a, "BOCA", "MULTISYNC", 800, 600, &(inittab[BOCA_800a]) },
 { BOCA_800c, "BOCA", "CRYSTALSCAN", 800, 600, &(inittab[BOCA_800c]) },
 { BOCA_800i, "BOCA", "INTERLACED", 800, 600, &(inittab[BOCA_800i]) },
 { BOCA_640, "BOCA", "STDVGA", 640, 480, &(inittab[BOCA_640]) },
};
...
ScrInfoRec vendorInfo = {
	...
	boca_init,	/* mode init - also called everytime during vt switch */
	boca_restore,	/* Restore() */
	boca_selectpage, 	/* SelectReadPage() */
	boca_selectpage,	/* SelectWritePage() */
	NoEntry,		/* AdjustFrame() */
	NoEntry,		/* SwitchMode() */
	NoEntry,		/* PrintIdent() */
...
/*
 * boca_init: Initialize any vendor specific extended register 
 * initialization here
 *
 * Before initializing the extended registers, save the current state
 */
boca_init (SIScreenRec *siscreenp)
{
	int saved_miscattr;
	unchar	temp;
	extern int inited;
	 ...
	 */
		if ((ioctl(vt_fd, KDADDIO, (unsigned short) 0x3bf) == -1) ||
		   ...
		   ErrorF("Can't enable BOCA extensions, KDADDIO Failed.\n");
	...
	case BOCA_1024i:
		(void)inb(vendorInfo.ad_addr + IN_STAT_1); /* init flip-flop */
		out_reg(&regtab[I_ATTR], 0x16, 0x00);
			if (v256_is_color) {
...
/*
 * Restore all the extended registers that were initialized in boca_init
 */
boca_restore(mode)
int mode;
{
...
/*
 * boca_selectpage(j)	-- select the current page based on the
...
#if 0
int
boca_select_readpage(int offset)
{
...
int
boca_select_writepage(int offset)
{
