...
export enum debug_level generic_omm_debug = DEBUG_LEVEL_NONE; 
/*
...
#define	OMM_CURRENT_MEMORY_MANAGER_DECLARE()			\
	...
	struct omm_memory_manager *memory_manager_p =		\
		screen_state_p->screen_memory_manager_state_p
#endif
...
omm_initialize(struct omm_initialization_options *omm_options_p)
{
	...
	if (omm_options_p)
	{
		...
		/*
		 * Parse named allocation string if present.
		 */

		if (omm_options_p->named_allocations_p != NULL)
		{
			...
			{
				(void) fprintf(stderr, OMM_MESSAGE_OPTION_PARSE_FAILED);
	...
	if (omm_options_p)
	{
		...
		while (tmp_named_allocate_node_p != NULL)
		{
					...
					memory_manager_p->total_depth))
			{
				...
				(void)fprintf(stderr,
					  OMM_MESSAGE_NAMED_ALLOCATE_GOES_OUT_OF_VIDEO_MEMORY);
