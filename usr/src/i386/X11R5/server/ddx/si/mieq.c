...
#ident	"@(#)r5server:ddx/si/mieq.c	1.3"
...
mieqProcessInputEvents ()
{
	...
	extern BOOL AXKeyboard;	
	extern BOOL KanaLockOn;
	...
	else
	{
		...
		if(KanaLockOn) {
			save_state = inputInfo.keyboard->key->state;
			inputInfo.keyboard->key->state &= ~Mod1Mask;
	    		(*miEventQueue.pPtr->processInputProc)
					    (&xe, miEventQueue.pPtr, 1);
			inputInfo.keyboard->key->state = save_state;
		}
		else {
	    		(*miEventQueue.pPtr->processInputProc)
					    (&xe, miEventQueue.pPtr, 1);
