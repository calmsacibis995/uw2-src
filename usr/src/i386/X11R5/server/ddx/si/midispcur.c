...
#ident	"@(#)r5server:ddx/si/midispcur.c	1.5"
    ...
    miPointerScreenFuncPtr  screenFuncs;
{
    ...
    pScreenPriv = (miDCScreenPtr) xalloc((unsigned long)sizeof(miDCScreenRec));
    if (!pScreenPriv)
	return FALSE;
    ...
    CursorPtr	pCursor;
{
    ...
    pPriv = (miDCCursorPtr) xalloc((unsigned long)sizeof(miDCCursorRec));
    if (!pPriv)
    ...
    int		x, y, w, h;
{
    ...
    pWin = WindowTable[pScreen->myNum];
    if (!pSave || (int)pSave->drawable.width < w || (int)pSave->drawable.height < h)
    {
	if (pSave)
    ...
    unsigned long   source, mask;
{
	...
	pScreenPriv->pTemp = pTemp = (*pScreen->CreatePixmap)
	    (pScreen, w, h, pScreenPriv->pSave->drawable.bitsPerPixel);
	if (!pTemp)
	    return FALSE;
    }
