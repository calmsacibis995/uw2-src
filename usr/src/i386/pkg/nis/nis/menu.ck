#ident	"@(#)pkg.nis:nis/nis/menu.ck	1.6"
#ident  "$Header: $"
...
`echo  "\n \n  NIS domain: $def_dom"`
`[ "$slavep" = "F" ] && echo  "\n \n  NIS servers for domain $def_dom: $SERV1"`
`[ "$slavep" = "F" ] && echo  "                                       $SERV2"`
`[ "$slavep" = "F" ] && echo  "                                       $SERV3"`
`[ "$slavep" = "T" ] && echo  "\n \n  NIS master server for domain $def_dom: $master"`
...
on the subsequent menu. Regardless of whether or not NIS configration is
