...
/	actual or intended publication of such source code.
	...
	.ident	"@(#)kern-i386at:psm/ast/spl.s	1.7"
	.ident	"$Header: $"
...
/
	.globl  ast_curmask     / arry of current masks
	.globl  ast_picmask     / array of pic masks
	.globl  ast_int_handle  / array of interrupt
	.globl  ast_calltab     / table of callable EBI II addresses

	.globl	picdeferred	/ deferred interrupt stack
	.globl	picdeferndx	/ deferred interrupt stack index
	.globl	myengnum	/ engine number of executing engine
...
ENTRY(splunwind)
	...
	movl	myengnum, %edx
	movl	picdeferndx(,%edx,4), %ecx	/ %ecx = top of deferred int stack
...
/
/ enableint() enables an interrupt by clearing its mask bit in ast_picmask[0],
...
ENTRY(enableint)
	...
	btrl	%ecx, ast_picmask	/ clear mask bit in ast_picmask[0][pic]

	xorl	%eax, %eax		/ load masks for level in pics
	movb	ipl, %al
...
/ disableint() disables an interrupt by setting its mask bit in ast_picmask[0],
/ and reloading the pic masks.
...
ENTRY(disableint)
	...
	movl    8(%ebp), %ecx           / get interrupt index arg
	btsl	%ecx, ast_picmask	/ set mask bit in ast_picmask[0][pic]
...
/
/       if ((mask = ast_picmask[ipl][myengnum]) == ast_curmask[myengnum])
/               return;
/
/       ast_curmask[myengnum] = mask;
/       (ast_calltab.RegSetLocalIntMask)(ast_int_handle[myengnum], mask);
/
/ note that the EBI function takes it's arguments in %esi and %eax
/
/ This function has been hand optimised from code which
/ was originally generated by the compiler.  If any changes
/ are made to the declarations of ast_int_handle, ast_picmask
/ or ast_calltab, they may affect the code here.  Please be
/ careful when changing code in ast.c.
/
/ If I knew more about optimising for the pentium I would have
/ done it here.  Feel free to make this code faster.
/
/ Note that this function clobbers %edx, %eax and %ecx, which is
/ what the i8259 versions of this code do also.
/

ENTRY(setpicmasks)
	movl	%eax, picipl                  / set new picipl
	movl	myengnum,%ecx                 / %ecx = myengnum
	
	/ get offset for this engine in ast_int_handle, ast_curmask and
	/ column of ast_picmask
	leal	(,%ecx,4),%ecx				  
	...
	/ fetch the two masks
	movl	ast_curmask(%ecx),%edx
	movl	ast_picmask(%eax,%ecx),%eax
	
	/ are they equal?
	cmpl	%edx,%eax
	je	.setpicmasksret

	/ no, set the new picmask
	pushl	%esi
	movl	%eax,ast_curmask(%ecx)
	movl	ast_int_handle(%ecx),%esi
	call	*ast_calltab+_A_REGSETLOCALINTMASK
	popl	%esi
