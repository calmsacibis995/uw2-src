...
/	actual or intended publication of such source code.
	...
	.ident	"@(#)kern-i386at:psm/pcmp/intr_p.s	1.5"
	.ident	"$Header: $"
...
/* starting apic interrupt vector handlers *//
ENTRY(ivct60)
	INTR_ENTER(0x60)
	incl	intr_count			/ count the device interrupt
...
ENTRY(.dev_common)
	/ may need to send a soft intr through the apic
	pushl	%ecx			/ interrupt number
	call	apic_selfintr
	popl	%ecx
...
ENTRY(.apic_common)
	...
	je	.stray_intr		/ issue EOI and return 
	cmp 	$apic_spur, %eax
...
.int_nosti:
	...
	movl	%eax, %edx
        movl     apic_primask(,%edx,4), %edx  / TASKPRI 8 bits only
...
/	movb	(%eax), %dl
	...
	movl	eoi_reg_addr, %eax
	movb	$0, %dl
	xchgb	%dl, (%eax)		/ sending EOI to APIC
        popl    %eax
