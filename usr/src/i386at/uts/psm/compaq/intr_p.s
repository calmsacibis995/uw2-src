...
/	actual or intended publication of such source code.
	...
	.ident	"@(#)kern-i386at:psm/compaq/intr_p.s	1.5"
	.ident	"$Header: $"
...
.devint0c:
	...
	testb	$0x02, sp_iomode	/ if jumpered in symmetric mode
	jnz	.dev_common		/	do dev_common
...
/
/ If its an inter-processor interrupt, set the INTDIS bit which
/ must be cleared by the interrupt handler (psm_intr). This is
/ necessary to prevent spurious inter-processor interrupts while
/ one is being handled. However, we can achieve the same functionality
/ by enabling interrupts (sti) when we enter the inter-processor
/ interrupt handler (psm_intr). The following code is here for
/ reference and uses the INTDIS approach to disable ipis.
/
/	cmpl	$13, %ecx
/	jne	.int_continue
/	pushl	%eax
...
/
/ PERF:
/	The code can be further optimized by doing an INTDIS
/	in .devint13, thus preventing a cmp and jmp instruction
/	for non-IRQ13 interrupts. However, we would have to clear
/	the INTDIS bit in the interrupt handler (psm_intr).
/
/ NOTE:
/	The spurious inter-processor interrupt problem is seen on
/	a Systempro XL while running in Systempro Compatible mode.
/
/
...
ifdef(`DEBUG',`
	/ check iomode -- symmetric or asymmetric
	testb	$0x01, sp_iomode	/ if jumpered in asymmetric mode
	jnz	.dodevxcall		/	handle xcall interrupt
	/
	/+ The Systempro is set to an illegal symmetric I/O mode. Only
	/+ Systempro/XL can be configured into symmetric mode.
	/
