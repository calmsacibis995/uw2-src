...
/	actual or intended publication of such source code.
	...
	.ident	"@(#)kern-i386at:psm/compaq/spl.s	1.6"
	.ident	"$Header: $"
...
ifdef(`DEBUG',`
	...
	jnz	.dosplunwind		/	do splunwind
	/
	/ must not have deferred interrupts for non-boot engine
	/ in asymmetric mode. Assert running on boot engine.
	/
...
LABEL(`setpic',`done')
	...
	testb	$ 0x02, sp_iomode	/ if (jumpered in symmetric mode ||
	jne	setpic			/
	cmpl	$ 0, myengnum	/ 	engine is boot engine) then
	jne	done			/	set pic masks
setpic:
...
ENTRY(picreload)
	cmpl	$0, myengnum		/ check if on boot engine
	je	.dopicreload		/	do picreload
	testb	$0x02, sp_iomode	/ if jumpered to asymmetric mode
	jz	.picreload_ret		/	return
.dopicreload:
	pushfl				/ save flags
...
ifdef(`DEBUG',`
	/ check iomode -- symmetric or asymmetric
	testb	$0x02, sp_iomode	/ if jumpered in symmetric mode
	jnz	.dosetpicmasks		/	do setpicmasks
	/
	/ must not do pic manipulation for non-boot engines while
	/ in asymmetric mode. Assert running on boot engine.
	/
        ASSERT(ul,`myengnum',==,$0)
.dosetpicmasks:
')
	...
	movl	iplmaskp(,%edi,4), %ebx
	leal	(%ebx,%eax,1), %ebx     / &iplmask[ipl][0]
	movl	curmaskp(,%edi,4), %esi          / &curmask[0]
...
.picloop:
	...
	movl	myengnum, %edi
	movl	iplmaskp(,%edi,4), %edi
	orb     (%edi,%ecx,1), %al      / OR in level zero mask
