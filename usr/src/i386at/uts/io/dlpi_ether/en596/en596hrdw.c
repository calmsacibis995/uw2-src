...
#ident	"@(#)kern-i386at:io/dlpi_ether/en596/en596hrdw.c	1.9"
#ident	"$Header: $"
...
extern	physreq_t	en596physreq;
extern	DL_irq_t	en596irq_group[];
extern	int		DLstrlog;
extern	int		en596boards;
extern	int		en596timer_id;

extern	int	en596wait_scb();
extern	int	en596init_596();
extern	void	en596init_board();
extern	void	en596init_chip();
extern	void	en596port();
extern	ushort	ntohs();
extern  ushort  en596config_596();

STATIC	void	en596tx_done();
STATIC	void	en596sched();
STATIC	void	en596ru_restart();
STATIC	int	disable_board();
STATIC	int	enable_board();
STATIC	int	reset_board();

void	en596bdspecioctl(), en596bdspecclose(), en596bdspecopen();
void	en596watchdog();
void	en596intr();
void	en596free_fcn();
int	en596xmit_packet();
...
/* en596bdspecopen is called from DLopen->dlpi_ether.c */
/* ARGSUSED */
void
en596bdspecopen(q)
queue_t *q;
...
/* en596bdspecclose is called from DLclose->dlpi_ether.c */
/* ARGSUSED */
void
en596bdspecclose(q)
queue_t *q;
...
/* en596bdspecioctl is called from DLioctl->dlpi_ether.c */
void
en596bdspecioctl(q, mp)
queue_t *q;
mblk_t *mp;
{
	...
	qreply(q, mp);
} /* end of en596bdspecioctl */


/*	This driver implements semi-dynamic chaining. en596xmit_packet() is
 ...
 *	the waiting list pointed by bdd->tail_tcb->next.
...
DL_sap_t *sap;
{
	 ...
	 * we will pick them up later in the en596sched().
	 */
	else {
	...
	if ( start_cu ) {
		if (en596wait_scb (bdd, 1000)) {
			cmn_err(CE_WARN,
				"en596xmit_packet scb command not cleared");
...
int	irq;
{
	...
	for (i = 0; i < DLboards; i++) {
		if ((bd = en596irq_group[irq].bd[i]) == (DL_bdconfig_t *)NULL) {
#ifdef DEBUG
			if (i == 0)
				cmn_err (CE_WARN, "en596 spurious interrupt");
#endif
			return;
		}
		...
		while (1) {
			...
			if (en596wait_scb(bdd, 1000)) {
				cmn_err(CE_WARN, "en596intr cmd not cleared");
...
exit_intr:
		...
		if (bd->io_start >= 0x1000) {		/* EISA boards */
			reg0 = inb(bdd->Control0);
			if (reg0 & PLX_R0_LSTAT)
...
start_cu:
		if (en596wait_scb (bdd, 1000)) {
			cmn_err(CE_WARN,
				"en596tx_done: scb command not cleared");
			bd->timer_val = 0;	/* force board to reset */
			return;
		}
		bd->timer_val = 2;
	...
	for ( i = bd->ttl_valid_sap; i; i-- ) {
		...
		if ( sap->state == DL_IDLE ) {
			while ( mp = getq(sap->write_q) ) {
				(void) en596xmit_packet(bd, mp, sap);
				bd->mib.ifOutQlen--;
				if( bd->flags & TX_BUSY ) {
...
DL_bdconfig_t   *bd;
{
	...
	if ( !(rfd->status & CS_COMPLETE) )
	    /* frame reception is still going on */
	    break;
	...
	if ((rfd->status & CS_OK) && (rbd->status & CS_EOF)) {
...
#ifdef DEBUG
		cmn_err(CE_WARN, "en596chk_ring: alloc failed.");
#endif
	     ...
	     */
	    cmn_err(CE_WARN, "!en596: rcv error %x", rfd->status);
	    bd->mib.ifInDiscards++;     /* SNMP */
	}
	rfd->status = 0;
...
DL_bdconfig_t   *bd;
{
	 ...
	 * en596chk_ring () and thus calls en596ru_restart() which will
	 * then start the RU if necessary.
	 */
	if ( !(scb->status & RU_NO_RBD) )
	 ...
	 */
	if (en596wait_scb(bdd,1000)) {
		cmn_err (CE_WARN, "en596: ru-restart, scb command not cleared");
		bd->timer_val = 0;	/* force board to reset */
		return;
	}
...
DL_bdconfig_t	*bd;
{
	...
	if (bd->promisc_cnt++)
		...
		rval = en596config_596 (bd, PRO_ON, LOOP_OFF);
...
DL_bdconfig_t	*bd;
{
	...
	if (--bd->promisc_cnt)
		...
		rval = en596config_596 (bd, PRO_OFF, LOOP_OFF);
...
DL_eaddr_t	*eaddr;
{
	...
	/* do IA Setup command */
	val = en596ia_setup_596(bd, bd->eaddr.bytes);
...
/*ARGSUSED*/
en596is_multicast(bd, eaddr)
DL_bdconfig_t *bd;
DL_eaddr_t *eaddr;
{
    bdd_t	*bdd = (bdd_t *)bd->bd_dependent1;
    mcat_t	*mcp = &bdd->en596_multiaddr[0]; 
    int		i;
    int		rval = 0;
...
DL_bdconfig_t	*bd;
{
	...
	if (!(bd->flags & BOARD_DISABLED)) {
		...
		outl(bdd->ChanAtn,(ulong) 0);
		if (en596wait_scb(bdd, 1000)) {
			cmn_err(CE_WARN, "en596 reset command failed");
			return -1;
		}
...
DL_bdconfig_t   *bd;
{
	...
	if (en596init_596(bd)) {
		cmn_err(CE_WARN, "en596 board %d init failed", bd->bd_number);
		return (-1);
	}
	drv_usecwait(1000);
...
{	
    ...
    for (i=0, bd=en596config; i < en596boards; bd++, i++) {
	...
	if (bd->timer_val == 0) {
	     ...
	     *+ debug message
	     */
	    cmn_err (CE_NOTE,"en596: board %d timed out.", bd->bd_number);
...
en596dump()
{
	...
	dumparea  = (char *)((uint)en596dump_buf & 0xFFFFFFF0);
