...
#ifndef _IO_DLPI_ETHER_EN596_H /* wrapper symbol for kernel use */
#define	_IO_DLPI_ETHER_EN596_H /* subject to change without notice */

#ident	"@(#)kern-i386at:io/dlpi_ether/en596/en596.h	1.6"
#ident  "$Header: $"
...
/*	The receive buffer is size is set to 1536 as a fix for ERRATA 26.
 	Description:
	...
	Workaround:
		The receive buffer is set to 1536, larger than the biggest
...
/*
 * Transmit command parameters structure
 */
typedef struct {
	paddr_t		tbd_paddr;	/* Transmit Buffer Descriptor address */
	ushort		tcb_count;	/* 82596 TCB embedded data count */
	ushort		zeros;		/* 82596 reserved field */
	net_addr_t	dest;
	ushort		length;		/* pad to 16-byte boundary */
} xmit_t;
...
#define	MAX_TBD		8	/* must be multiple of 4 in cogent */
...
typedef struct rbd {
	...
	caddr_t		rxbuf;
	ulong		pad;		/* to align rbd to 16 byte boundary */
} rbd_t;
...
typedef struct fd {
	...
	ulong		pad1[2];	/* to align rfd to 16 byte boundary */
} rfd_t;
...
typedef struct bdd {
	...
	tcb_t		*head_tcb;	/* beginning of the pending tx cmd */
	...
	int		first_init;
	DL_sap_t        *next_sap;
	mcat_t		en596_multiaddr[MULTI_ADDR_CNT];
	net_addr_t	eaddr;
...
#define EN596_TIMEOUT	(3*HZ)		/* 3 sec */
...
#define	PLX_R0_EDGE_INT	0x1	/* bit 0, 1=edge triggered, 0=level	*/
...
#define	PLX_R1_D_ISA_IO	0xe0	/* ISA addressing disable		*/
