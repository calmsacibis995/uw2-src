...
#ident	"@(#)kern:fs/memfs/memfs_mnode.h	1.10"
#ident	"$Header: $"
 ...
 *
 *	(3) The corresponding page of the file is held PAGE_USELOCKED and
...
#ifndef NO_RDMA
	ushort_t	mnt_page_flags;		/* P_DMA or P_NODMA */
#endif
#ifdef _MEMFS_HIST
...
#define mno_page_flags		mno_top.mnt_page_flags
...
/*
 * void
 * MNODE_INIT_PAGE_FLAGS(mnode_t *mp)
 *	Initialize the page flags of an mnode.
 *
 * Calling/Exit State:
 *	mp is privately held.
 */
#ifdef NO_RDMA
#define MNODE_INIT_PAGE_FLAGS(mp)	/* nothing to do in this case */
#else /* !NO_RDMA */
#define MNODE_INIT_PAGE_FLAGS(mp)	{				\
	(mp)->mno_page_flags = P_NODMA;					\
}
#endif /* NO_RDMA */
...
#define MNODE_INIT_COMMON(mp, size) {					\
	...
	(mp)->mno_size = (size);					\
	MNODE_INIT_PAGE_FLAGS(mp);					\
	(MNODE_TO_VP(mp))->v_op = &memfs_vnodeops;			\
...
/*
 * void
 * MNODE_INIT(mnode_t *mp, vattr_t vap, cred_t crp)
 *	Initialize a named mnode.
 *
 * Calling/Exit State:
 *	mp is privately held.
 */
#define MNODE_INIT(mp, vap, cred) {					\
	FSPIN_INIT(&(mp)->mno_mutex);					\
	(mp)->mno_mode = MAKEIMODE((vap)->va_type, (vap)->va_mode);	\
	(mp)->mno_flags = 0;						\
	(mp)->mno_flag = 0;						\
	(mp)->mno_uid = (cred)->cr_uid;					\
	(mp)->mno_gid = (cred)->cr_gid;					\
...
/*
 * uint_t
 * MEMFS_PAGE_FLAGS(mnode_t *mp)
 *	Return the appropriate flags for use with page_lookup_or_create3()
 *	for the specified mnode.
 *
 * Calling/Exit State:
 *	None.
 */
#ifdef NO_RDMA
#define MEMFS_PAGE_FLAGS(mp)	0
#else /* !NO_RDMA */
#define MEMFS_PAGE_FLAGS(mp)	((mp)->mno_page_flags)
#endif /* NO_RDMA */
