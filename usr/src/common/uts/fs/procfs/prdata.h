...
#ident	"@(#)kern:fs/procfs/prdata.h	1.19"
#ident	"$Header: $"
...
typedef struct prnode {
	lock_t		pr_mutex;	/* Locks pr_flags and child pr_files */
	...
	struct vnode	*pr_next;	/* List in chain of (invalid) vnodes */
 ...
 */
#define prisempty(sp) setisempty((ulong_t *)(sp), sizeof(*(sp))/sizeof(ulong_t))
