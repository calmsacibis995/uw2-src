...
#define xp_sock		xp_fd
	...
	struct xp_ops {
		...
		bool_t	(*xp_getargs)(struct __svcxprt *, xdrproc_t, caddr_t);
		bool_t	(*xp_reply)(struct __svcxprt *, struct rpc_msg *);
		bool_t	(*xp_freeargs)(struct __svcxprt *, xdrproc_t, caddr_t);
...
extern	bool_t		svc_reg(SVCXPRT *, u_long, u_long,
...
extern	int		svc_run_parallel (int, int, int, size_t);
...
extern	SVCXPRT		*svc_tp_create (void (*)(struct svc_req *, SVCXPRT *),
				u_long, u_long, struct netconfig *);
#else
...
extern	SVCXPRT 	*svc_tli_create (int, struct netconfig *,
				struct t_bind *, u_int, u_int);
#else
