...
#ident	"@(#)kern:net/nw/spx/test/tli/getinfo.c	1.3"
...
#ident	"$Id: getinfo.c,v 1.2 1994/02/18 15:06:14 vtag Exp $"
/*********************************************************************
*
*	Program Name:	
*	File Name:		getinfo.c
...
*	Purpose:			Test the TLI t_getinfo function.
...
*	getinfo
*
*		This routine tests the TLI t_getinfo routine.
*
*	Return:	None
*
*	Parameters:
...
void getinfo (char *protocol, int mode, int major)
{
	...
	showProtoStk("TESTING t_getinfo",mode);

/*--------------------------------------------------------------------
*	Make a valid t_getinfo call  
*-------------------------------------------------------------------*/
	showtnum(major,minor);
	fd = tliopen(protocol,mode,NULL,TNOERR,UNBND);
	tligetinfo(fd,&info,TNOERR,UNBND);
	checktinfo(&info);
...
*-------------------------------------------------------------------*/
	...
	minor++;
	showtnum(major,minor);
	tligetinfo(fd,&info,TBADF,UNINIT);
}
