...
#ifndef _NET_TIHDR_H	/* wrapper symbol for kernel use */
#define _NET_TIHDR_H	/* subject to change without notice */

#ident	"@(#)kern:net/tihdr.h	1.18"
#ident	"$Header: $"
...
#define XPG4_1          0x0004  /* provider confirms to XTI in XPG4 and *
...
struct queue;			/* For user level usage */

/* connect response */

struct T_conn_res {
	long    PRIM_type;	/* always T_CONN_RES       */
	struct queue *QUEUE_ptr;/* responding queue ptr    */
	long    OPT_length;	/* options length          */
	long	OPT_offset;	/* options offset          */
