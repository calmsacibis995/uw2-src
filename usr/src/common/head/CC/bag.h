...
public:
    inline Bagiter(const Bag<T>&);
    inline Bagiter(const Bagiter<T>&);
    inline ~Bagiter();
    const Bag_pair<T>* first();
    const Bag_pair<T>* next();
    inline int next(const Bag_pair<T>*&);
    inline void reset();
    inline const Bag<T>* the_bag() const;
...
//  Relations
    ...
    inline int operator==(const Bag<T>&) const;
    inline int operator!=(const Bag<T>&) const;
    inline int operator<=(const Bag<T>&) const;
    inline int operator<(const Bag<T>&) const;
    inline int operator>=(const Bag<T>&) const;
...
//  Bag algebra
    ...
    inline Bag<T> operator|(const Bag<T>&) const;
    inline Bag<T> operator-(const Bag<T>&) const;
    inline Bag<T> operator&(const Bag<T>&) const;
    inline Bag<T> operator^(const Bag<T>&) const;
    Bag<T>& operator|=(const Bag<T>&);
...
//  Bag_bucketiter_ATTLC<T> inlines

template <class T>
void
Bag_bucketiter_ATTLC<T>::clobber()
{
    pos.curr_depth = -1;
}
...
template <class T>
Bagiter<T>::Bagiter(const Bag<T>& b) :
    Bag_bucketiter_ATTLC<T>(b),
    inited(0),
    itp(0)
{ }

template <class T>
Bagiter<T>::Bagiter(const Bagiter<T>& bi) :
    Bag_bucketiter_ATTLC<T>(bi),
    inited(0),
    itp(0)
{ }
...
template <class T>
int
Bagiter<T>::next(const Bag_pair<T>*& t)
{
    t = next();
    return t!=0;
}
...
template <class T>
unsigned int
Bag<T>::size_unique() const
{
    return (unsigned)sze_unique;
}

template <class T>
Bag<T>::operator const void*() const
{
    return (size()? (this): (0));
}
