...
public:
    ...
    inline void clobber();
    Bucketiter_ATTLC& operator=(const Bucketiter_ATTLC<T>&);
};
...
private:
    ...
    Listiter<T>* itp;
public:
    inline Setiter(const Set<T>&);
    inline Setiter(const Setiter<T>&);
    inline ~Setiter();
    const T* first();				// undocumented - TBD_make private
    const T* next();				// subsumes first, next
    inline int next(const T*&);			// new
    inline void reset();			// new semantics
    inline const Set<T>* the_set() const;	// new
...
//  Size
    ...
    inline unsigned int size_unique() const;
...
//  Relations
    ...
    inline int operator==(const Set<T>&) const;
    inline int operator!=(const Set<T>&) const;
    inline int operator<=(const Set<T>&) const;
    inline int operator<(const Set<T>&) const;
    inline int operator>=(const Set<T>&) const;
...
//  Membership
    ...
    inline unsigned int count(const T&) const;

//  Insert and Remove elements

    const T* insert(const T&, int=1);
    unsigned remove(const T&, int=1);
    inline unsigned remove_all(const T&);
    unsigned remove_all();
...
//  Set algebra
    ...
    inline Set<T> operator|(const Set<T>&) const;
    inline Set<T> operator-(const Set<T>&) const;
    inline Set<T> operator&(const Set<T>&) const;
    inline Set<T> operator^(const Set<T>&) const;
    Set<T>& operator|=(const Set<T>&);
...
//  Bucketiter_ATTLC<T> inlines

template <class T>
void
Bucketiter_ATTLC<T>::clobber()
{
    pos.curr_depth = -1;
}
...
template <class T>
Setiter<T>::Setiter(const Set<T>& s) : Bucketiter_ATTLC<T>(s), inited(0), itp(0)
{
}

template <class T>
Setiter<T>::Setiter(const Setiter<T>& si)
...
template <class T>
unsigned int
Set<T>::size_unique() const
{
    return (unsigned)sze;
}

template <class T>
Set<T>::operator const void*() const
{
    return (size()? (this): (0));
}
...
Set<T>::space() const
{
    ...
    if( !contents.is_null() ) {
	for ( int i=0 ; i<SET_POSITIONS_ATTLC ; i++) {
	    result.leaves[i] = 0;
