...
#ident	"@(#)ucb:common/ucbcmd/man/man.c	1.9"
#ident	"$Header: $"
...
#define	SUBSEC1	"bcdfghimprsvwx"	/* subsections to try in section 1 */
#define	SUBSEC2	"bdhivwx"
#define	SUBSEC3	"abcdefghijklmnprstvwx"
#define	SUBSEC4	"bdfhimnpsvx"
#define	SUBSEC5	"bdhivx"
#define	SUBSEC7	"bdhitvx"
#define	SUBSEC8	"bcvs"
#define SUBSEC0 ""
	...
	char *argv[];
{
	...
	label = (char *) malloc(strlen("UX:")+strlen(cmdname)+1);
	if (label == NULL) {
		label = "UX:man";
	} else {
		(void) strcpy(label, "UX:");
		(void) strcat(label, cmdname);
	}
...
usage:
	...
	if (troffit) {
		...
		if (((pager = getenv("PAGER")) == NULL) || (*pager == NULL)) {
			pager = MORE;
			/* If standard more is not installed, use pg */
			if ((access("/usr/bin/more",X_OK)) < 0)
				pager = "pg";
		}
	}
...
sectin:
	...
	/*
	 * If no entry is found, make sure there is at least a MANDIR
	 * unless user specified a different MANPATH.
	 */

	if (mrc && !strcmp(manpath, MANDIR)) {
		struct stat st_info;

		if ( stat(MANDIR, &st_info) || (st_info.st_mode & S_IFDIR == 0) ) {
			pfmt(stderr, MM_WARNING, ":1006:Traditional manual pages are not installed in %s.\n", MANDIR);
			pfmt(stderr, MM_ACTION,  ":1007:Use the DynaText Document Broswer to access graphical\n\tmanual pages, or install the Traditional Manual Pages package\n\t(manpages) to access character based manual pages.\n");
			pfmt(stderr, MM_ACTION,  ":1008:Packages can be installed using the App_Installer\n\ticon in the Admin_Tools folder, or the pkgadd command.\n\tSee the Installing and Sharing Applications chapter in the\n\tSystem Owner Handbook.\n");
		}
	}
...
search:
		...
		if (!checkpage()) {
			...
			(void) fprintf(stderr,
				       "No entry for %s in section %c", name,
				       section);
			rc = 6;
			if (subsec) {
				(void) fprintf(stderr, "%c", subsec);
				rc = 7;
			}
			(void) fprintf(stderr, " of the manual.\n");
			return(rc);
		}
	}
...
again:
	...
	while (*bp && *cp && lmatch(*bp, *cp))
		bp++, cp++;
	if (*cp == 0 && (*bp == '(' || *bp == ',' || *bp == '\t' || *bp == ' ' || *bp == ':'))
		return (1);
	while (*bp != '(' && *bp != ',' && *bp != '\t' && *bp != ' ' && *bp != ':' && *bp)
		bp++;
	if ((*bp != ',') && (*bp != ':'))
		return (0);
	bp++;
