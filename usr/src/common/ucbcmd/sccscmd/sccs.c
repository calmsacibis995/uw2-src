...
#ident	"@(#)ucb:common/ucbcmd/sccscmd/sccs.c	1.5"
#ident	"$Header: $"
	...
	*/
	...
	switch (cmd->sccsoper)
	{
	  ...
	  case ENTER:		/* enter new sccs files */
		 ...
		 * complain.
		 */
		if (SccsDir[0] != '\0' && SccsPath[0] != '/' &&
		    strncmp(SccsPath, "./", 2) != 0)
		{
			extern void *malloc();
			p = malloc(strlen(SccsDir) +
				   strlen(SccsPath) + 2);
			sprintf(p, "%s/%s", SccsDir, SccsPath);
		}
		else
			p = SccsPath;
		if (stat(p, &statb) == -1) 
