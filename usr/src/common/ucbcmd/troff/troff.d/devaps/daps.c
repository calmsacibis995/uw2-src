#ident	"@(#)ucb:common/ucbcmd/troff/troff.d/devaps/daps.c	1.4"
#ident	"$Header: $"
...

{
	...
	signal(SIGFPE, (void (*)())float_err);			/* catch floating point errors */

	if ( signal(SIGINT, (void (*)())wrap_up) == SIG_IGN )  {	/* ignoring interrupts */
		signal(SIGINT, SIG_IGN);					/* so reset SIGINT */
		signal(SIGQUIT, SIG_IGN);					/* and ignore the rest */
		signal(SIGHUP, SIG_IGN);
	} else {										/* wrap_up() handles them */
		signal(SIGQUIT, (void (*)())wrap_up);
		signal(SIGHUP, (void (*)())wrap_up);
	}	/* End if */
