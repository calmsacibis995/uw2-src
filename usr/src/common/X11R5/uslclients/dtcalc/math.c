...
void fail_op()
{
	...
	strcpy(dispstr, GetGizmoText(TXT_DIV_BY_ZERO));
	break;
      case FPE_INTOVF:		/* integer overflow */
      case FPE_FLTOVF:		/* floating point overflow */
	strcpy(dispstr, GetGizmoText(TXT_OVERFLOW));
	break;
      case FPE_FLTUND:		/* floating point underflow */
	strcpy(dispstr, GetGizmoText(TXT_UNDERFLOW));
	break;
      case FPE_FLTRES:		/* floating point inexact result */
	strcpy(dispstr, GetGizmoText(TXT_INEXACT_RESULT));
	break;
      case FPE_FLTINV:		/* invalid floating point operation */
	strcpy(dispstr, GetGizmoText(TXT_INVALID_OP));
	break;
      case FPE_FLTSUB:		/* subscript out of range */
	strcpy(dispstr, GetGizmoText(TXT_OUT_OF_RANGE));
	break;
...
#endif
           default:               
		strcpy(dispstr, GetGizmoText(TXT_ERROR));
    }
    else 
	if (SignalKind == SIGILL)
	    strcpy(dispstr, GetGizmoText(TXT_ILLEGAL_OPERAND));
...
{
  if (opsp==STACKMAX) {
			strcpy(dispstr, GetGizmoText(TXT_STACK_ERROR));
			entered=3;
