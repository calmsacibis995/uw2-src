     ...
     int *width, *height, *x_hot, *y_hot;
{
  ...
  status = XReadBitmapFile(dpy, RootWindow(dpy, screen), filename, (unsigned int *)width,
			   (unsigned int *)height, &bitmap, x_hot, y_hot);
     ...
     int width, height;
{
  ...
  if (!XGetGeometry(dpy, d, &root, &x, &x, (unsigned int *)&x, (unsigned int *)&x, (unsigned int *)&x, (unsigned int *)&depth))
    return(0);
     ...
     char *name;
{
	...
	if (!XQueryTree(dpy, top, &dummy, &dummy, &children, (unsigned int *)&nchildren))
	  return(0);
