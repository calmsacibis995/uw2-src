...
#ident	"@(#)r5extensions:server/xtest1dd.c	1.3"
...
{	
	...
	{ 
		 ...
		 */
		dev_type = XTestUnpackDeviceID((char)request->action_list[parse_index]);
		/*
...
{	
	...
	action_array[write_index].device = XTestUnpackDeviceID((char)fkey->header);
...
{	
	...
	dy = (XTestUnpackYMotionValue((char)fmotion->motion_data));
	...
	action_array[write_index].device = XTestUnpackDeviceID((char)fmotion->header);
...
XTestJumpInfo	*fjump;
{
	...
	pmousey = fjump->jumpy;
	action_array[write_index].type = XTestJUMP_ACTION;
	action_array[write_index].device = XTestUnpackDeviceID((char)fjump->header);
	action_array[write_index].x = pmousex;
	action_array[write_index].y = pmousey;
