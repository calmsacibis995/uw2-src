...
#ident	"@(#)r5extensions:server/shm.c	1.7"
    ...
    register ClientPtr client;
{
    ...
    if (stuff->format == XYBitmap)
    {
        if (stuff->depth != 1)
            return BadMatch;
        length = PixmapBytePad((int)stuff->totalWidth, 1);
    }
    ...
    else if (stuff->format == ZPixmap)
    {
        if (pDraw->depth != stuff->depth)
            return BadMatch;
        length = PixmapBytePad((int)stuff->totalWidth, stuff->depth);
    }
    else
    {
    ...
    if ((int) (stuff->srcX + stuff->srcWidth) > (int)stuff->totalWidth)
    {
	client->errorValue = stuff->srcWidth;
	return BadValue;
    }
    if ((int) (stuff->srcY + stuff->srcHeight) > (int)stuff->totalHeight)
    {
    ...
    if ((((stuff->format == ZPixmap) && (stuff->srcX == 0)) ||
	 ((stuff->format != ZPixmap) &&
	  ((int)stuff->srcX < screenInfo.bitmapScanlinePad) &&
    ...
    register ClientPtr client;
{
	...
	if (stuff->x < 0 ||
	    ...
	    stuff->y < 0 ||
	    stuff->y+(int)stuff->height > (int)pDraw->height
	    )
	    return(BadMatch);
	xgi.visual = None;
    }
    ...
    {
	length = PixmapBytePad((int)stuff->width,pDraw->depth) * stuff->height;
    }
    else 
    {
	lenPer = PixmapBytePad((int)stuff->width, 1) * stuff->height;
...
CreatePmap:
    VERIFY_SHMSIZE(shmdesc, stuff->offset,
		   PixmapBytePad((int)stuff->width,stuff->depth)*stuff->height,
		   client);
