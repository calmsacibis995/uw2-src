...
int
ErrorHandler(Display *dsp, XErrorEvent *theError)
{
	char	msg[80];

	if (theError->request_code == X_ChangeHosts ||
		theError->request_code == X_SetAccessControl) {
		return;
	}
	XmuPrintDefaultErrorMessage(dsp, theError, stderr);
	exit(1);
}
...
getPassword()
{
    ...
    if (pw == NULL)  {
#ifdef PDEBUG
	fprintf(stderr,"Bad password structure for uid: %d\n",uid);
#endif
	return 1;
	}
    }
    ...
    /*
     * Throw away all events in the input queue. XGetWindowAttributes() 
     * doesn't seem to set up the window attributes structure ( xgwa ) 
     * if there is an Async reply in the input queue, even though the 
     * return is non-zero.  This causes a core dump later on in the draw 
     * routine, ( worm_doit modules using wp->xsize which is 0 )
     * XError message without the XSync is:
     *        Xlib: unexpected async reply (sequence 0xXXXX)!
     * Core dump sigfpe.
     */
    XSync(dsp,True);
	 ...
	 */
#ifdef PDEBUG
	fprintf(stderr,"crypt rootpass: %s\n",crypt(buffer, rootpass));
	fprintf(stderr,"rootpass: %s\n",rootpass);
	fprintf(stderr,"userpass: %s\n",userpass);
	fprintf(stderr,"crypt userpass: %s\n",crypt(buffer, userpass));
	fprintf(stderr,"allowroot=%d\n",allowroot);
#endif
	...
	if (!done && *buffer == NULL) {
#ifdef PDEBUG
	fprintf(stderr,"just hit return so break\n");
#endif
...
#ifdef PDEBUG
	fprintf(stderr,"userpass is null and buffer is null so set done to false\n");
#endif
	}
...
#ifdef PDEBUG
	fprintf(stderr,"return 1 from GetPassword\n");
#endif
    return 1;
}
...
justDisplay()
{
	...
	/*
	 * Get the mouse position so we can use it later
	 */
	XQueryPointer(dsp,RootWindow(dsp,DefaultScreen(dsp)),
			&root_return,&child_return,&root_x_return,&root_y_return,
			&x_return,&y_return,&mask_return);
	x = root_x_return;
	y = root_y_return;
	...
	if (event.type == MotionNotify)
	{
		XQueryPointer(dsp,RootWindow(dsp,DefaultScreen(dsp)),
			&root_return,&child_return,&root_x_return,&root_y_return,
			&x_return,&y_return,&mask_return);
		if ( x >= root_x_return + 20 || x <= root_x_return - 20 )
			break;;
		if ( y >= root_y_return + 20 || y <= root_y_return - 20 )
			break;;
	}

    } while (event.type != ButtonRelease && event.type != KeyPress);;
    ...
    char       *argv[];
{
	...
	xswa.event_mask = KeyPressMask | ButtonReleaseMask | VisibilityChangeMask
							| PointerMotionMask;
...
#define CWMASK CWOverrideRedirect | CWBackPixel | CWEventMask
	...
	xswa.event_mask = ButtonReleaseMask;
...
#define CIMASK CWBorderPixel | CWBackPixel | CWEventMask
	...
	XForceScreenSaver(dsp, ScreenSaverReset);	/* In case we were blanked out already */
    }
#ifndef DEBUG
    GrabKeyboardAndMouse();
#endif
