 ...
 *
 * Revision History:
 * 19-Aug-93: added uname test in open_display
...
#define DEF_FONT	"-b&h-lucida-medium-r-normal-sans-17-*-*-*-*-*-iso8859-1"
#define DEF_BG		"White"
#define DEF_FG		"Black"
    ...
    caddr_t    *valuep;		/* RETURN */
{
	...
	*((float *) valuep) = (float) atof(buffer);
	...
	text_name = GetStr(TXT_username);
	text_pass = GetStr(TXT_password);
	text_info = GetStr(TXT_info);
	text_valid = GetStr(TXT_validate);
	text_invalid = GetStr(TXT_invalid);
}
...
open_display()
{
	...
	machine.nodename[0] = 0;    /* just in case set to a non-valid first char */
	uname(&machine);
    ...
    char       *argv[];
{
	...
	largc = argc;
	/*
	 * Copy all the user arguments into our arg buffer.  
	 */
	if (( largv = (char **)malloc(argc * sizeof(char *))) == NULL )
	{
		exit(1);
	}
	for ( i = 0; i < argc; i++ )
	...
	/*
	   We need to give precedence to the command line arguments
	   over the ~/.Xdefaults file.  Parsing the command args twice
	   because up above parsing is done to get the display variable
	   if any. Redo it here to overwrite the .Xdefaults values.
	*/
    cmdlineDB = NULL;
    XrmParseCommand(&cmdlineDB, cmdlineTable, cmdlineEntries, ProgramName,
		    &largc, largv);
