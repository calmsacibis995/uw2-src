...
#ident	"@(#)r5fonts:server/difs/dispatch.c	1.4"
    ...
    ClientPtr   client;
{
    ...
    int client_timeout = client->clientGone == CLIENT_TIMED_OUT;

    if (client_alive || client->clientGone == CLIENT_TIMED_OUT) {
    ...
    if (client_alive || client->clientGone == CLIENT_GONE) { 
	--nClients;
