...
#ident	"@(#)r5fonts:server/difs/main.c	1.3"
    ...
    char      **argv;
{
	...
	if (serverGeneration == 1) {
	    ...
	    CreateSockets(ListenSock,namedFd);
	    InitProcVectors();
	...
	/* clean up per-cycle stuff */
        ...
        if(dispatchException & DE_RESET) {
           int i;
           StopListening();
           InitFonts(); /* to close the libraries */
           CloseErrors();
           for(i = 3; i < lastfdesc; i++)
              close(i);
           execv(argv[0],argv);  /* re-exec myself to really clean up */
        }
