...
inline const UString& DirectoryEntry::name() const { return *name_; }

class DirectoryImpl {
private:
    friend class Directory;

    DirectoryImpl(DIR*, UString*);
    ~DirectoryImpl();
    ...
    static Uboolean dot_slash(const char*);
    static Uboolean dot_dot_slash(const char*);
    static const char* home(const char*);
    static const char* eliminate_dot(const char*);
    static Uboolean collapsed_dot_dot_slash(char*, char*& start);
    ...
    static Uboolean ifdir(const char*);
};
...
Directory* Directory::open(const UString& name) {
    UString* s = canonical(name);
...
const UString* Directory::path() const {
    DirectoryImpl& d = *impl_;
    return d.name_;
}
...
const UString* Directory::name(int i) const {
    DirectoryImpl& d = impl_->filled();
    if (i < 0 || i >= d.count_) {
...
int Directory::index(const UString& name) const {
    NullTerminatedUString ns(name);
...
Uboolean Directory::is_directory(int i) const {
    DirectoryImpl& d = impl_->filled();
    if (i < 0 || i >= d.count_) {
	/* raise exception -- out of range */
	return False;
...
inline Uboolean DirectoryImpl::dot_slash(const char* path) {
    return path[0] == '.' && (path[1] == '/' || path[1] == '\0');
}

inline Uboolean DirectoryImpl::dot_dot_slash(const char* path) {
    return (path[0] == '.' && path[1] == '.' &&
	(path[2] == '/' || path[2] == '\0')
    );
}

UString* Directory::canonical(const UString& name) {
    NullTerminatedUString ns(name);
    ...
    return new CopyUString(newpath);
}

Uboolean Directory::match(const UString& name, const UString& pattern) {
...
DirectoryImpl::DirectoryImpl(DIR* d, UString* name) {
    dir_ = d;
    entries_ = NULL;
    count_ = 0;
    used_ = 0;
...
void DirectoryImpl::do_fill() {
#ifdef apollo
    for (struct direct* d = readdir(dir_); d != NULL; d = readdir(dir_)) {
#else
	...
	e.name_ = new CopyUString(d->d_name);
	e.info_ = NULL;
	++used_;
    }
...
Uboolean DirectoryImpl::collapsed_dot_dot_slash(char* path, char*& start) {
...
const char* DirectoryImpl::interpret_tilde(const char* path) {
    ...
    Uboolean valid = (beg != NULL && (beg == path || *(beg - 1) == '/'));
    if (valid) {
	const char* end = strchr(beg, '/');
	int length = (end == NULL) ? strlen(beg) : (end - beg);
...
Uboolean DirectoryImpl::ifdir(const char* path) {
    struct stat st;
