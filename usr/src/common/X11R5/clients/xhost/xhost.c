...
#undef STREAMSCONN
#ifndef TCPCONN /* added to stop the macro redefined: TCPCONN warning */
#define TCPCONN
#endif
#endif
...
#include <netdir.h>
#include <tiuser.h>
static struct netbuf * GetHostServiceByName();
#define FamilyNetWare 99
#endif /* NOVELL */
#ifdef NEEDSOCKETS
#ifdef att
    ...
    Bool add;
{
   ...
   */
    ...
    struct netbuf* n_buf;
    char *display_num;
    display_num = strchr(DisplayString(dpy),':') + 1;
       if(( n_buf = GetHostServiceByName( name, *display_num )) == 0 )
          return 0; 
       else {
          ha.family = FamilyNetWare;     /* the transport is ipx or spx */
          ha.length = strlen(name) + 1; /* os/access.c doesn't yet understand */
          ha.address = name;           /* the addresses for ipx/spx so I'm */
    	  if (add) {	              /* using FamilyNetWare and passing */
	      XAddHost (dpy, &ha);   /* the nodename + a null-character */
          } else {
	      XRemoveHost (dpy, &ha);
          }
       }
...
#endif
#ifdef NOVELL
  if( ha->family == FamilyNetWare ) {
      char *nm = (char *)malloc(ha->length + 1);
      strncpy(nm, ha->address, ha->length);
      return nm;
  }
#endif /* NOVELL */
#ifdef STREAMSCONN
...
GetHostServiceByName(host,display)
char *host;
char display;
{
   int     i;
   ...
   if((handlep = setnetpath()) == NULL ) {
      t_error("Error in network initialization");
      return 0;
   }
   sprintf(service , "xserver%c",  display );
   ...
   while((netconfigp = getnetpath(handlep)) != NULL) {
      if(strcmp(netconfigp->nc_proto, "spx") && 
           strcmp(netconfigp->nc_proto, "ipx") )
                continue;
