     ...
     hlPtr data;
{
  ...
  for (i=1; i<ncolors; i++) {
    if ((int)Intensity((float)mptr) < (int)Intensity((float)tptr)) 
      mptr = tptr;
    tptr++;
  }
  return mptr->pixel;
...
static Widget pane1, pane2, pane3, cclose, replace, new, selection, paste, label;
     ...
     hlPtr data;
{
  ...
  selection = XtCreateManagedWidget("select", commandWidgetClass, pane2,
			      (Arg *) NULL, 0);
  XtAddCallback(selection, XtNcallback, SelectCB, (XtPointer)data);
