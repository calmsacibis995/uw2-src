     ...
     */
{
	...
	sw->scale.table.width[(int) x] = (Dimension)
	    (Position)myrint(sw->scale.scale_x * (x + 1)) - (Position)myrint(sw->scale.scale_x * x);
    }
	...
	sw->scale.table.height[(int) y] = (Dimension)
	    (Position)myrint(sw->scale.scale_y *(y + 1)) - (Position)myrint(sw->scale.scale_y * y);
    }
}
