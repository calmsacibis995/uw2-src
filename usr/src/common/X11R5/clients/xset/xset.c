    ...
    int special, before, after;
{
    ...
    if ( before == 1 || after == 1 ) {
           /* make sure that this isn't a duplicate entry */
    register char **cp = currentList;
    register int i = 1;

       while (i++ <= ncurrent) {
       register char *cp1 = strdup(path);
       register char *to_free = cp1;
		 if((cp1= strtok( cp1, "," )) != NULL ) {
			if(strcmp(cp1,*cp) == 0) {
			  free(to_free);
                          exit(3);
                        }
                    while ((cp1 = strtok (NULL, ",")) != NULL) {
	               if(strcmp(cp1,*cp) == 0) {
                          free(to_free);
                          exit(3);
                       }
