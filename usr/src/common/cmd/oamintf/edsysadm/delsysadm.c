...
#ident	"@(#)oamintf:common/cmd/oamintf/edsysadm/delsysadm.c	1.3.8.6"
...
int errcode;
{
	...
	/*
	**      delete temporary files which we may have created
	*/
	unlink(tempmenu);
	unlink(exprlog);
	unlink(chngmenu);
...
int taskpick;
{
	...
	/* end of ifit's a menu */
		...
		if ((tagged != NULL) && (*tagged  != NULL)) {
			...
			if (!taskdir[0])
				(void) sprintf(fulldir,"%s%s%s%s%s%s",oampath,PATHDEL,ADDONS,PATHDEL,grabtag,relpath);
			else
				(void) sprintf(fulldir,"%s",taskdir);
		} else {
...
char *newreal;			/* original tmp file, becomes new parent menu */
{
	...
	errlink = unlink(old_real);     /* get rid of temp file */
        if(errlink < 0)
        {
                /* error with unlinking temp file */
                return(-1);
        }
	return(0);
}	/* end of parnt_xfer */
