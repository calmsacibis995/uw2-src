...
/*LINTLIBRARY*/
#ident	"@(#)oamintf:common/cmd/oamintf/libintf/iexpr.c	1.4.6.3"
...
int	*not_o, *o;
{
	...
	*not_o = *o = 0;
	...
	for (found = 0, y = thread_strt; y != (TASKDESC *)0; y = y->thread) {
		++found;
		if((y->action != NULL && *(y->action) == 'O') && (*(y->action + 1) == '\0')) {
			++*o;
		} else {
			++*not_o;
		}
...
int	first_fit;	/* this level MUST at least partial match */
{
	...
	for (y = p1; y != (TASKDESC *)0; y = y -> next) {
		...
		if((i == strlen(y->tname)) && (strncmp(seed, y->tname, i) == 0)) {
			/* partial match at this point in hierarchy */
			if(partial) {
				/* if only partial, then was this OLD_SYSADM branch */
				if(y->action != NULL && strcmp(y->action, "O") == 0) {
					add_thread(y);
					y->mark = 2;
				} else if(y->action != NULL && strcmp(y->action, "R") == 0) {
					add_thread(y->child);
					y->mark = 2;
				} else {
				...
				/* complete match */
				if ( y->action != NULL && strcmp(y->action, "R") == 0 ) {
					add_thread(y->child);
					return(1);
				} else {
					found = 1;
