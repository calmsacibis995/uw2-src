...
/*LINTLIBRARY*/
#ident	"@(#)oamintf:common/cmd/oamintf/libintf/read_item.c	1.3.6.4"
...
int full_line;			/* flag to read & store full line or not*/
{
	...
	unsigned char *lineptr;			/* pointer to line structure */
	...
	menu_ptr->next = NULL;
	lineptr = (unsigned char *)menu_ptr->line;
	...
	if(fgets(menu_ptr->line, sizeof(menu_ptr->line), fileptr) == NULL)
		...
		while(!done) {
			...
			lineptr-=2;
			if ( lineptr < (unsigned char *)menu_ptr->line ) lineptr = (unsigned char *)menu_ptr->line;

			while((lineptr >= (unsigned char *)menu_ptr->line )&&(isspace(*lineptr--)));
			lineptr++;
			if ( lineptr < (unsigned char *)menu_ptr->line ) lineptr = (unsigned char *)menu_ptr->line;
			...
			if(*lineptr == '\\') { /* continuation: get next line */
				if(full_line) {
					...
					curptr = curptr->next;
					lineptr = (unsigned char *)curptr->line;
				}
				else lineptr = (unsigned char *)newline;
				if(fgets((char *)lineptr, sizeof(curptr->line), 
					fileptr) == NULL) done = 1;
			}
			else done = 1;
