...
#ident	"@(#)listen:listen.c	1.19.21.13"
...
int			Free_call_alloc = 0; /* mallocs in dequeue */
...
char **argv;
{
	...
	if ((i = open(log, O_RDWR|O_CREAT|O_APPEND, 0644)) < 0)
		logexit(1, nologfile);
	...
	strcpy(Provbuf, Netconf->nc_device);
 ...
 */
	...
	logmessage("@(#)listen:listen.c	1.19.21.8");
...
register struct callsave *cp;
{
	...
	if ((head == Free_call_p) && Free_call_alloc) {
		DEBUG((9,"free in queue"));
		Free_call_alloc--;
		t_free (cp->c_cp, T_CALL);
		free(cp);
		return;
	}
...
register struct call_list *head;
{
	...
	if (head->cl_head == (struct callsave *) NULL)  {
		if (head == Free_call_p) {
			sprintf(scratch, "mallocing callsaves ");
			logmessage(scratch);
			...
			for (dbp=Dbfhead, i=0; dbp && i < Dbf_entries; dbp++, i++) {
				if (dbp->dbf_fd >= 0) {
					bindfd = dbp->dbf_fd;
					break;
				}
			...
			Free_call_alloc++;
			DEBUG((9, "malloc in dequeue"));
			return((struct callsave *) tmp);
		}
		else {
			DEBUG((9, "NULL return"));
...
char **ap;
{
	...
	state = t_getstate(fd);
	DEBUG((9,"in bind, fd = %d, clen = %d, state = %d", fd, clen, state));
	...
	if (clen)  {
		...
		pbuf[req->addr.len] = (char)0;
		state = t_getstate(fd);
		DEBUG((3,"bind: fd=%d, address=%c%s%c, len=%d, state=%d",
			fd, '\"',pbuf, '\"', req->addr.len, state));
#endif	/* CHARADDR  && DEBUGMODE */
...
listen()
{
	...
	for (;;) {
		...
		if (poll(Pollfds, Valid_addrs + 1, -1) < 0) {
			...
			for (i = 0, sp = Pollfds; (i < Valid_addrs + 1) && (sp->fd != -1); i++, sp++) {
				switch (sp->revents) {
				case POLLIN:
					if (sp->fd == Pmpipefd) {
...
int fd;
{
	...
	char scratch[BUFSIZ];
	/* include callers address in log file*/
	char caller[512];
	dbf_t *dbp;
	...
	DEBUG((9, "case t_listen "));
		...
		if (t_listen(fd, call) < 0) {
			DEBUG((9, "t_listen failed"));
			queue(Free_call_p, current);
			tli_error(E_T_LISTEN, CONTINUE);
			return (-1);
		}
		dbp = getentry(fd);
		sprintf(scratch, "New call on addr <%s> fd %d, seq # %d",
			dbp->dbf_svc_code, fd, call->sequence);
		logmessage(scratch);

		memcpy(caller, "unknown", 8);
		if(call->addr.buf && call->addr.len) {
			if ((call->addr.len) > 255)
				nlsaddr2c(caller, call->addr.buf, 255);
			else
				nlsaddr2c(caller, call->addr.buf, call->addr.len);
		}
		sprintf(scratch, " Caller : %s", caller);
		logmessage(scratch);
	...
	DEBUG((9, "case t_disconnect"));
		if (disc == NULL) {
			while (!(disc = (struct t_discon *)t_alloc(fd, T_DIS, T_ADDR|T_OPT)) ) {
		   		if (t_errno == TBADF)
...
register dbf_t *dbp;
{
	...
	/*
	 * The pushmod below will establish a controlling terminal 
	 * for this process if any of the modules pushed are 'tty'
	 * type devices. Modules that are 'tty' devices send up
	 * SO_ISTTY. This is the first chance we get to do this
	 * since the TLI connection to this point could not be one
	 * and the fork/setpgrp done above would have removed any
	 * controlling terminal anyway.
	 *
	 * You will need to have pushed a 'tty' type device if the
	 * service and or scheme that will be run expects a 'tty'
	 * type device (and thus a controlling terminal). (ttymon
	 * is a special case since it does the pushing later itself).
	 * You don't have to worry about this if the service is a pure
	 * TLI type service
	 */
