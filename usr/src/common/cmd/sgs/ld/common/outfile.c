...
#ident	"@(#)ld:common/outfile.c	1.55"
/*
...
build_osymtab()
{
	...
	if (Gflag) { /* will hash the large symbol table */ 
		...
		chain = &hashtab[2 + dynbkts];
		hashtab[0] = dynbkts;
		hashtab[1] = count_hidden + count_outglobs + count_outlocs + 2;
	}
				...
				if (locsym->ls_syment->st_shndx == SHN_ABS)
                        symptr->st_shndx = SHN_ABS;
                           ...
                           symptr->st_shndx = SHN_UNDEF;
                                   else if ( locsym->ls_scnptr && locsym->ls_scnptr->is_outsect_ptr )
	...
	/*  If we have any symbols to hide in a shared object, add the hidden 
	**  symbols to the to the output symbol table and change the TYPE to 
	**  STB_LOCAL from STB_GlOBAL.
	*/
	
	if (dmode && Gflag && count_hidden ){ 
		/* Add a file symbol, _fake_hidden,  to the symtab */
