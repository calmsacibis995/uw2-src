...
#ident	"@(#)ld:common/sections.c	1.39"
/*
** Module sections
...
make_hash()
{
	...
	if(Gflag)
		size = my_elf_fsize(ELF_T_WORD, (size_t)(count_hidden +
		     count_outglobs + count_outlocs + 3 + 2 + dynbkts), libver);
	else
...
make_strtab()
{
	...
	count_strsize += strlen(outfile_name)+2;	/* one for the null byte at end of filename */
							/* one for the null byte at beginning of section */
	if (count_hidden)
		count_strsize += strlen(FAKE_NAME) + 1; /* add in the 
				special symbol _fake_hidden so that the
...
make_symtab()
{
	...
	nsyms = count_hidden + count_outglobs + count_outlocs + 3;
