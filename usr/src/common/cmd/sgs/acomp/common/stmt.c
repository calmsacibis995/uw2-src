...
#ident	"@(#)acomp:common/stmt.c	55.4.2.13"
/* stmt.c */
...
/* Add case to current switch statement. */
{
	...
	*/
	...
	if (TY_SIZE(cur_swtype) > TY_SIZE(cur_swotype)) {
	    p = tr_conv(p, cur_swtype, gettxt(":740","sm_sw_case1??"), TR_CAST);
	    ...
	    */
	    ...
	    in_chkval(&node, cur_swtype, (int) TY_SIZE(cur_swtype), 0,
			gettxt(":315","case label affected by conversion"));	/*ERROR*/
	    /* Truncate expression type to controlling expression type. */
	    p = tr_conv(p2, cur_swtype, gettxt(":741","sm_sw_case2??"), TR_CAST);
	    val = tr_inconex(p);
	}
	else {
	    p = tr_conv(p, cur_swtype, gettxt(":742","sm_sw_case3??"), TR_CAST);
