...
#ident	"@(#)acomp:common/init.c	55.1.1.8"
/* init.c */
	...
	*/
	...
	if (asize != TY_NULLDIM) {
	    ...
	    */
	    ...
	    if (stsize > asize) {
		WERROR(gettxt(":236","%d extra byte(s) in string literal initializer ignored"),
			stsize-asize);
		stsize = asize;
	    }
	}
	...
	*/
	if (TY_ISINTTYPE(t) && TY_ISNUMTYPE(p->type)) {
		...
		in_chkval(p, t, (int) II(in_sp).in_size, isfield,
			gettxt(":237","initializer does not fit"));	/*ERROR*/
	}
	else {
...
*/
{
	...
	*/
	...
	WERROR(gettxt(":239","%s: %#x"), mesg, (UCONVAL) val);
    else
	WERROR(gettxt(":240","%s: %ld"), mesg, val);
    return;
}
