...
#ident	"@(#)acomp:common/trees.c	55.12.3.27"
/* trees.c */
...
*/
{
	...
	*/
	...
	for (end = s; *end; ++end) {
	    ...
	    */
	    if (*end > '7')
		WERROR(gettxt(":330","bad octal digit: '%c'"), *end);
	    result <<= 3;
	    result += *end - '0';
	}
    }
...
*/
{
	...
	if (p->type == TY_NONE) {
	    ...
	    */
		...
		if (verbose)
#ifndef LINT
		    WERROR(gettxt(":338","implicitly declaring function to return int: %s()"), SY_NAME(p->rval));
#else
		...
		UERROR(gettxt(":339","undefined symbol: %s"), SY_NAME(p->rval));
	    ...
	    */
		...
		WERROR(gettxt(":340","using out of scope declaration: %s"), SY_NAME(p->rval));
...
*/
{
    ...
    if (TY_ISFTN(p->type)) {
	if (p->op == NAME)
		UERROR(gettxt(":341","cannot take sizeof function: %s"), SY_NAME(p->rval));
	else
	...
	WERROR(gettxt(":343","cannot take sizeof bit-field: %s"), SY_NAME(p->right->rval));
    else {
	T1WORD t;
	t = p->type;
	if(p->op == CALL || p->op == UNARY CALL || p->op == INCALL ||
		p->op == UNARY INCALL ) {
		T1WORD ptype;
		ptype = p->left->type;
...
/* build a UNARY AND node */
{
    ...
    */
	...
	UERROR(gettxt(":358","cannot take address of bit-field: %s"),SY_NAME(l->right->rval));
		...
		UERROR(gettxt(":359","%s: %s"), gettxt(":405",addrmesg), SY_NAME(l->rval));
	    else
		WERROR(gettxt(":359","%s: %s"), gettxt(":405",addrmesg), SY_NAME(l->rval));
	}
		...
		(SY_FLAGS(l->left->rval) & SY_ISREG)){
		WERROR(gettxt(":359","%s: %s"), gettxt(":405",addrmesg), SY_NAME(l->left->rval));
	}
    }
    else
...
/* convert "array of types" to "pointer to type" with U& parent */
{
	...
	*/
	...
	if (p->op == NAME && (SY_FLAGS(p->rval) & SY_ISREG)) {
	    WERROR(gettxt(":372","cannot take address of register: %s"), SY_NAME(p->rval));
...
*/
{
	...
	/*FALLTHRU*/
    case TY_CHAR:
	if (chars_signed && v & ONEBIT(TY_SIZE(TY_CHAR)-1) )
	    return ( v | ~BITMASK(TY_SIZE(TY_CHAR)) );
	/*FALLTHRU*/
    case TY_UCHAR:
	return ( v & BITMASK(TY_SIZE(TY_CHAR)) );
