...
#ident	"@(#)lprof:libprof/common/exist.c	1.2.1.7"
#include <stdio.h>
#include <string.h>
#include <pfmt.h>
...
char *covfile;
{
	...
	filedata = (struct caFILEDATA *) _lprof_Malloc(1, sizeof(struct caFILEDATA));
	if (filedata != NULL)
...
debug(	    	fprintf(stderr,"_CAfind: another time, offset==%lo(octal)\n",
		...
		name = (char *) _lprof_Malloc(1, fname_size+1);
...
static char *string_warn0 =
"uxcds:1370: Profiled object file names are different, but time stamps match.\n";
static char *string_warn1 =
"uxcds:1371:\tAssuming correct object file.***\n";
...
struct  caOBJ_ENTRY *obj_entry;
{
	...
	if (
		...
		if (
			...
			pfmt(stderr,MM_ERROR, string_warn0);
			pfmt(stderr,MM_NOSTD, string_warn1);
			goto success;
		}
	} else {
		pfmt(stderr,MM_ERROR,COV002);
	}
	return(COVOBJ_FAIL);
...
debugp1("\n")
	...
	dataptr->func_name = (unsigned char *)_lprof_Malloc(1, dataptr->fname_size+1);
	if (
		fread(
...
debugp3("_CAread: function name read; is: ", dataptr->func_name, "\n")
	...
	if (
		...
		dataptr->lca_counts = (caCOVWORD *)
			_lprof_Malloc(1, dataptr->lca_words*(sizeof(caCOVWORD)));
...
struct caFILEDATA *filedata;
{
	...
	{
		pfmt(stderr,MM_ERROR,"uxcds:1347:_CAvalid_test: %s\n",gettxt("uxcds:1303",COV303));
		ret_code = BUG_FAIL;
	 }
...
struct caOBJ_ENTRY *obj_ent1, *obj_ent2;
{
    ...
    {
	ret_code = BUG_FAIL;
	pfmt(stderr,MM_ERROR, "uxcds:1348:_CAobj_entry: %s\n",gettxt("uxcds:1301",COV301));
    }
    return(ret_code);
}
