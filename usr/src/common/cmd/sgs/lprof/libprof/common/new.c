...
#ident	"@(#)lprof:libprof/common/new.c	1.2.1.5"
			...
			if (
				...
				while(count--)
					if(
						...
						free((char *) filedata);
						filedata = NULL;
						pfmt(stderr,MM_ERROR,"uxcds:1349:_CAcreate_covf: %s\n",gettxt("uxcds:1301",COV301));
					}
			 }
			else
			{
...
/*	void	rewind();	*/
	...
	if (filedata->obj_cnt == no_obj)
	{
		...
		{
			/* can't read header */
			pfmt(stderr,MM_ERROR,"uxcds:1350:_CAcomp_covf: %s\n",gettxt("uxcds:1303",COV303));
			ret_code = COMP_FAIL;
		 }
	 }
	else
...
struct caCOV_DATA *data;
{
	...
	if ((unsigned int)filedata->obj_cnt >= 1) {
		...
		if (
			...
			{
				ret_code = BUG_FAIL;
				pfmt(stderr,MM_ERROR,"uxcds:1351:_CAdata_entry: %s\n",gettxt("uxcds:1301",COV301));
			}
		 } else {
			ret_code = BUG_FAIL;
			pfmt(stderr,MM_ERROR,"uxcds:1351:_CAdata_entry: %s\n",gettxt("uxcds:1301",COV301));
...
char *objname;
{
	...
	if (ofileptr != NULL)
	{
	   if((unsigned int)filedata->obj_cnt < OBJMAX)
	   {
	    ...
	    {   pfmt(stderr,MM_ERROR,COV304);
		ret_code = FULL_FAIL;
   	    }
	}
	else
	{	pfmt(stderr,MM_ERROR,COV305);
		ret_code = OBJ_FAIL;
	}
