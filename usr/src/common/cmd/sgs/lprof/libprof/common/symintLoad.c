...
PROF_FILE	*proffilePtr;
{
	...
	prsym_list_p = (PROF_SYMBOL *) _lprof_Calloc(st_size, 1);
...
Elf32_Sym *end_p;
{
	...
	if (add_profsym_search_fail) {
		_err_exit(gettxt("uxcds:1357","Unable to locate file %s in debug information.\n"),
			ST_NAME(filsym_p)
		);
	}
	...
	if (priorFile_dbp && (priorFile_dbp->pd_file_p == NULL)) {
...
int		*count_p;
{
	...
	list_p = (SEARCH_NODE *) _lprof_Calloc(count, sizeof(*list_p));
...
*		- _lprof_Calloc space to contain the pointers.
...
init_dblist()
{
	...
	dblist = (SEARCH_NODE *) _lprof_Calloc(dblist_cnt, sizeof(*dblist));
...
Elf32_Sym *prsym_p;
{
	...
	if (stchk_func(prsym_p)) {
		...
		if (
			...
			_err_warn(
				gettxt("uxcds:1358","File name %s was used more than once."),
				sn_p->sn_name_p
			);
		}
		add_file(&tag);
...
DB_TAG	*tag_p;
{
	...
	curf_lns_p = (PROF_LINE *) _lprof_Calloc(curf_lncnt, sizeof(*curf_lns_p));
...
DB_TAG		*tag_p;
{
	...
	*/
	if (ps_p->ps_dbg.pd_line_p == NULL) {
		_err_exit(
			gettxt("uxcds:1359","Unable to locate line information for function %s."),
			SYMBOL_NAME(ps_p)
		);
	}
	...
	if ( ++prsym_cnt > prsym_cap ) {
		if ( prsym_cap == 0 ) {
			...
			prsym_list_p = (PROF_SYMBOL *)
				_lprof_Calloc( prsym_size, prsym_cap );
		} else {
			prsym_cap += PS_BLKFACTOR;
			prsym_list_p = (PROF_SYMBOL *)
...
char	*attr_p;
{
	...
	if (len==0)
		_err_exit(gettxt("uxcds:1360","Invalid FORM_value %#x for attribute type %#x\n"),
			form , type);
...
verify_match()
{
	...
	if (_prof_check_match) {
		if (!(sn_p = sn_search(MATCH_STR, gstlist, gstlist_cnt))) {
			_err_exit(gettxt("uxcds:1361","Cannot find match name."));
		} 
		sym_p = (Elf32_Sym *) sn_p->sn_value_p;
		if (sym_p->st_value != (Elf32_Addr) &MATCH_NAME) {
			_err_exit(gettxt("uxcds:1362","Location of file for this process unknown."));
		}
	}
}
#endif
