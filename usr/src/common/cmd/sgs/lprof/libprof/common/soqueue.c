...
#ident	"@(#)lprof:libprof/common/soqueue.c	1.2.1.5"

#include <pfmt.h>		/* uses "_" to hide libc functions */
#include <unistd.h>		/* uses "_" to hide libc functions */
...
TYPE_BASEAD	base;
{
	...
	if (init_flag) {
		...
		if ((linkmap_p = get_lmap()) == NULL) {
			_err_exit(gettxt("uxcds:1354","Cannot find link map."));
		}
	}
	if ((link_p = find_linkentry(base)) == NULL) {
		_err_exit(gettxt("uxcds:1355","Cannot find linkmap entry shared object."));
	}
	...
	if (!(qn_p = search_queue((void *) link_p->l_name, chk_path))) {
		...
		qn_p = (QUEUE_NODE *) _lprof_Calloc(1, sizeof(*qn_p));
...
TYPE_BASEAD	base;
{
	...
	if (qn_p->qn_cfilename_p) {
		...
		if ((qn_p->qn_cfilename_p = tempnam(NULL, "lp")) == NULL) {
			_err_exit(gettxt("uxcds:1356","Unable to create temp name."));
		}
	}
...
*/
static char *str_dumping = "uxcds:1381:Dumping profiling data from process '%s' . . .\n";
...
_CApredump()
{
	...
	if (opts.msg == TRUE) {
		pfmt(stderr,MM_INFO, str_dumping, _CAproc);
	}
