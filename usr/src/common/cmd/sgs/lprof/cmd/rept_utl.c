...
#ident	"@(#)lprof:cmd/rept_utl.c	1.11.1.6"
...
struct command *envir;
{
	...
	if ( envir == NULL) {
		pfmt(stderr,MM_ERROR,":1263:CAreport: bad pointer passed\n");
		return(BUG_FAIL);
	}
...

#define COV_DATA_LOST_STR0 \
":1295:***coverage data for function '%s',object file '%s',\n"
#define COV_DATA_LOST_STR1 \
":1296:\tnot found in covfile '%s'***\n"
...
short type;
{
	...
	pfmt(	
	    stderr,MM_ERROR,
	    ":1264:***unable to access CNTFILE '%s'***\n",
	    envir->cov_ptr[0]
	);
	return(COND_FAIL);
    }
    ...
    if( validcode != OK) {
	_err_exit(gettxt(":0","Invalid cover file %s"),
	envir->cov_ptr[0]);
    }
    ...
    */
    ...
    if(_CAfind_obj(filedata, obj_name, &obj_buf) != OK) {
    	/* _CAfind_obj failed */
	pfmt(stderr,MM_ERROR,":1265:***no coverage data");
	if (obj_name != NULL)
	    pfmt(stderr,MM_NOSTD,":1266: for object file '%s'",obj_name);
	pfmt(stderr,MM_NOSTD,":1267: in CNTFILE '%s'***\n",envir->cov_ptr[0]);
	ret_code = COND_FAIL;
	goto theend;
    }
	...
	obj_name = obj_buf.name;
	if ((objfile = _symintOpen(obj_name)) == NULL) {
	    pfmt(stderr,MM_ERROR,":1268:***cannot access object file '%s'***\n",obj_name);
	    ret_code = COND_FAIL;
	    goto theend;
	}
    }
    if (elf_kind(objfile->pf_elf_p) == ELF_K_COFF) {
	    ...
	    ":1269: %s: Warning - internal conversion of COFF file to ELF\n",
	    obj_name
	    );
    }
    ...
    */
	...
	if ((src_list = CAsrc_list(envir, filedata)) == NULL) {
	    pfmt(stderr,MM_ERROR,":1270:No source files found.\n\n");
	    ret_code = SRC_FAIL;
	}
...
struct command *envir;
{
	...
	if (type == LINE || type == SUM)
	{
	    ...
	    funcdata.func_name = (char *)_lprof_Malloc(1, covdata.fname_size+1);
	    ...
	    free(funcdata.func_name);
		    ...
		    the function data */
	{
	    node = (struct caFUNC_DATA *) _lprof_Malloc(1, sizeof(struct caFUNC_DATA));
	    node->func_name = (char *) _lprof_Malloc(1, covdata.fname_size+1);
...
char *src, *obj;
{
	switch (type)
	{
		...
		case BLANK:
			break;	/* no header */
		default:
			pfmt(stderr,MM_ERROR,":1271:CArepthdr:  unrecognized report type\n");
			break;
	   }
  }
