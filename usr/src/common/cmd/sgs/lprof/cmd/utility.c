...
#ident	"@(#)lprof:cmd/utility.c	1.10.3.2"
...
struct caCOV_DATA *covdata_p;
{
	...
	char *func_not_found = ":1297:function '%s' not found\n";
	...
	for (;;) {
		...
		if ((sym_p = NEXT_SYMBOL(sym_p)) == oldsym_p) {
			pfmt(stderr,MM_ERROR, func_not_found, bufptr->func_name);
			return(FUNC_FAIL);
		}
	}
...
new_data_block()
{
	...
	p = (struct caDATA_BLK *) _lprof_Malloc(1, sizeof(struct caDATA_BLK));
	p->entry_cnt = 0;
	p->stats = (struct caSTAT_BLK *) _lprof_Malloc(1, sizeof(struct caSTAT_BLK));
	p->next_blk = NULL;
	return(p);
}
...
short code;
{
	...
	if(code != OK) {
		if((code & EDIT_MASK) != 0)
			pfmt(stderr,MM_WARNING,":1288:  PROFILING DATA HAS BEEN EDITED \n");
		if((code & VER_MASK) != 0)
			pfmt(stderr,MM_WARNING,":1289:  PROFILING DATA WAS CREATED WITH DIFFERENT VERSION OF LPROF/XPROF\n");
		if((code & MACH_MASK) != 0) {
			pfmt(stderr,MM_WARNING,":1290: CNTFILE MACHINE TYPE DOES NOT MATCH THIS MACHINE\n");
			pfmt(stderr,MM_NOSTD,":1291:\tDATA MAY BE UNINTELLIGIBLE, ERRORS ARE LIKELY\n");
		}
		if((code & COMP_MASK) != 0)
			pfmt(stderr,MM_WARNING,":1292:  CNTFILE IS INCOMPLETE, POSSIBLY BAD DATA \n");
	  }
...
struct caOBJ_ENTRY *objbufptr;
{
	...
	{
		pfmt(stderr,MM_WARNING,":1293:  OBJECT FILE'S TIME STAMP IS DIFFERENT THAN IT WAS WHEN PROFILING\n");
		pfmt(stderr,MM_NOSTD,":1294:\tDATA WAS RECORDED. IF FILE HAS BEEN CHANGED, ERRORS ARE LIKELY\n");
	   }
  }
