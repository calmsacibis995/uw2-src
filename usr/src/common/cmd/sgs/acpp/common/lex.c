...
#ident	"@(#)acpp:common/lex.c	1.31.3.4"
...
#endif
	COMMENT(cplusplus || (tk_eol(head.next))->rlen == 1);
	COMMENT(cplusplus || (tk_eol(head.next))->code == C_WhiteSpace);
	COMMENT(cplusplus || (tk_eol(head.next))->ptr.string[0] == '\n');
	return head.next;
}
#endif

