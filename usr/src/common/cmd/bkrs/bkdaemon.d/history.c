...
#ident	"@(#)bkrs:common/cmd/bkrs/bkdaemon.d/history.c	1.9.5.6"
...
history_m *msg;
{
	...
	if( msg->flags & HST_IS_TOC ) {
		...
		(void) bkTLassign( h_tid, eptr, H_ARCHTOC, "N", h );
...
argv_t *argv;
{
	...
	if (ptr = (char *)TLgetfield(tid, eptr, fieldname))
		ptr = (char *)strdup(ptr);
	else
		return(0);
	...
	for( i = 0; (*argv)[i]; i++ ) {
		for( j = 0; (*t_argv)[j]; j++ ) 
			if( (*argv)[i] && (*t_argv)[j] && !strcmp( (*argv)[i], (*t_argv)[j] ) ) {
