...
#ident	"@(#)bkrs:common/cmd/bkrs/bkdaemon.d/method.c	1.14.11.6"
...
int	m_slot;
{
	...
	/* Allocate devices */
	/* Need to call d_allocate() in order to find out */
	/* which device in the "dgroup" will be used if   */
	/* "ddevice" is not give.                         */
	if( !(owner->options & S_NO_EXECUTE) || !method->entry.ddevice) {
		...
		case BKBADTABLE:
			...
			brlog("jobid back-%ld tag %s: device not speicifed in device.tab",
		...
		case BKBUSY:
			...
			brlog( "Devices for jobid back-%ld tag %s are busy, try to spawn it later",
				oproc->pid, method->entry.tag );
                        (void)sprintf(msg.text.text,"%s: Devices(s) is being reserved by another backup, will try later",method->entry.tag);
                        bkm_send(oproc->pid, DEV_BUSY, data);
			return( rc );
...
method_t *method;
{
	...
	method->argv[ argc++ ] = (entry->oname == NULL) ? (char *)strdup("") :
		 (char *)strdup((char *)entry->oname);
	method->argv[ argc++ ] = (entry->odevice == NULL) ? (char *)strdup("") :
		(char *)strdup( (char *)entry->odevice );
	method->argv[ argc++ ] = (entry->olabel == NULL) ? (char *)strdup("") :
...
char *tag;
{
	...
	for( m_slot = owner->methods; m_slot > 0; m_slot = method->next_l ) {
		...
		if (!tag) tag = "";
		if (!method->entry.tag)
			method->entry.tag = (unsigned char *)strdup("");
