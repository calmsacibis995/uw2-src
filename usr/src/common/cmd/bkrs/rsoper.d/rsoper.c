...
#ident	"@(#)bkrs:common/cmd/bkrs/rsoper.d/rsoper.c	1.6.6.3"
...
char *method = "";      /* Method that we're using */           
char *oname = "";       /* Origination name */                  
char *odevice = "";     /* Origination device */                
char *jobids;
argv_t *jobs;
char *user = 0;	/* Do restore requests for a particular user */
char *label = "";       /* label on this volume */ 
	...
	if( flags & rFLAG ) {
		...
		*/
		...
		/* perform a restore operation */
                if (!get_arch_info( ddevice, dchar, &machine, &oname, &odevice, 
                                        &method, &label, &toc ))                
                {                                                               
                        fprintf(stderr,"Restore request has been terminated\n");
                        exit(0);                                                
                }                                                              
...
/*** BUG - list not set to NULL if there ***    
 *** is no entry in status table         ***/   
                if ( !rc )                      
                {                               
		...
		*/
		...
		case BKINTERNAL:
		case BKNOMEMORY:
                        bkerror( stderr, ERROR23, odevice, ddevice );   
			exit( 2 );
		...
		default:
			break;
                ...
                /* Now prune the list so that the candidates are all of   */ 
                /* the same type. Also mark them "ACTIVE" in the rsstatus */ 
                /* table                                                  */ 
                l_prune( &list, oname, odevice, method, label, toc );        

		rstm_init();

                if ( !strcmp( (char *)list->type,R_PARTITION_TYPE ) ||        
                     !strcmp( (char *)list->type, R_DISK_TYPE ) )             
                {                                                             
                        fprintf(stdout,"Attempting to restore %s\n",odevice); 
                }                                                             
                else                                                          
                {                                                             
                        fprintf(stdout,"Attempting to restore %s\n",oname);   
		...
		if( rc = rsspawn( list, oname, odevice, ddevice, dchar, dmnames, method,
			...
			exit( 2 );
                ...
                {                                                               
                        if ( !strcmp( (char *)lptr->status, (char *)RST_FAILED) )                                                                               
                        {                                                       
                                fprintf( stderr, "%s\n",(char *)lptr->explanation);                                                                             
                        }                                                       
                }                                                               
...
synopsis()
{
	...
	(void) fprintf( stderr, "\t[-u user] [-j jobids] [-o oname[:odevice]]\n" );
        (void) fprintf( stderr, "rsoper -r jobids\nrsoper -c jobids\n" );     
}
...
char *ddev;
{
	...
	if( !*dchar ) dchar = NULL;
        if( dmnames && !*dmnames ) dmnames = NULL;   
