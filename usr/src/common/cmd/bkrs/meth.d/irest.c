...
#ident	"@(#)bkrs:common/cmd/bkrs/meth.d/irest.c	1.18.5.6"
...
int		seq1_isopen;
{
	...
	mp->method_name = (mp->method_name == NULL) ? "" : mp->method_name;
	mp->ofsname = (mp->ofsname == NULL) ? "" : mp->ofsname;
	mp->ofsdev = (mp->ofsdev == NULL) ? "" : mp->ofsdev;
	mp->ofslab = (mp->ofslab == NULL) ? "" : mp->ofslab;
	...
	if (outdev == mp->nfsdev) {
		if (mp->nfsdevmnt) {
                        brlog("Cannot restore to mounted device %s",mp->nfsdev); 
                        sprintf(ME(mp), "Cannot restore to mounted device %s",mp->nfsdev);                                                                      
			if (ifd != NULL) {
				(void) g_close(ifd);
				ifd = NULL;
			}
		...
		if (mp->ofsdevmnt) {
                        brlog("Cannot restore to mounted device %s",mp->ofsdev); 
                        sprintf(ME(mp), "Cannot restore to mounted device %s",mp->ofsdev);  
			if (ifd != NULL) {
				(void) g_close(ifd);
				ifd = NULL;
			}
...
register file_rest_t	*f;
{
	...
	if (A_special) {
		if ((Stat.st_mode & S_IFMT) == S_IFIFO)
			Stat.st_rdev = 0;
                if (create_a_special(newname,f,np,Stat.st_mode,Stat.st_rdev, NULL)) {
			return(1);
		}
		...
		if (ftype == S_IFDIR) {
			...
			if (ret = chown(newname, Stat.st_uid, Stat.st_gid)) {
				...
				return(1);
                        ...
                        if (create_a_reg(newname, np, f, Stat.st_mode, &fd, NULL) < 0) {  
...
static int
try_em(nm, statptr, flags, unused)
const char		*nm;
