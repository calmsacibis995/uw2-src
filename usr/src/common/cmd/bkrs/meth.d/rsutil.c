...
#ident	"@(#)bkrs:common/cmd/bkrs/meth.d/rsutil.c	1.5.5.3"
	...
	if (safe_stat(namep, &Xstatb) == 0) { 	/* already exists */
		brlog("%s EXISTS mtime=0x%x, f_mtime=0x%x, restore date=0x%x",
			namep, Xstatb.st_mtime, f_mtime, fp->ldate);
                ...
                if ( (fp->date != NULL ) && (Xstatb.st_mtime) > fp->ldate) {  
#ifdef TRACE
...
int
create_a_special(namep, fp, np, mode, rdev, Dhead)
char            *namep;                           
char            *np;                              
file_rest_t     *fp;                              
mode_t          mode;                             
...
/* try creating (only twice) */
	...
	do {
        ...
        } while (ans < 2 && missdir(np, fp, Dhead) == 0); 
		...
		if (*np == '/') {
			...
			*np = '\0';
			...
			if (stat(namep, &Xstatb) == -1) {
                                if ((ctt = makdir(namep, fp, 0, 0, Dhead)) != 0) {                                                                              
					*np = '/';
					return(ctt);
				}
			}
...
int
create_a_reg(namep,np,fp,mode,fd,Dhead)  
char            *namep;                  
char            *np;                     
file_rest_t     *fp;                     
mode_t          mode;                    
...
gid_t			gid;
{
	...
	do {
        ...
        } while(ans < 2 && missdir(namep, fp, Dhead) == 0);  
