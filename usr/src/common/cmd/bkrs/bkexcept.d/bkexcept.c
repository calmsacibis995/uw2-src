...
#ident	"@(#)bkrs:common/cmd/bkrs/bkexcept.d/bkexcept.c	1.6.9.6"
	...
	while ( (pat = (*p_argv)[i++]) != NULL ) {
		if ( strcmp( pat, "-" ) == 0 ) {
			...
			if ((entryno = TLsearch1(tid, TLsearches, TLBEGIN, TLEND, TL_AND))
				!= TLFAILED) {
				/** Entry already exits, don't duplicate **/
...
printall()
{
	...
	/* ok for in-core sort */
		...
		/* entrycnt is number of non-commented entries to process
                 * i is number processed so far
                 */
                while ( i <= (entrycnt)
