...
#ident	"@(#)lp:cmd/lpsched/lpNet/bsdChild/recvjob.c	1.7.2.10"
		...
		*--cp = '\0';
		...
		case READCFILE:	/* read cf file */ 
			...
			if (dfile_in) {
			    ...
			    fname = cvt_spoolfn(ctf_hdr, CFILE);
			    print_request(rqp, fname);	   /* does putrequest(), etc. */
			    trashrequest(rqp);
			    free(cfile);
			    free(ctf_hdr);
			    return;
		...
		case READDFILE:	/* read df file */
			...
			if (cfile_in) {
			    if (!(rqp = mkrequest(cfile, cfile_in))) {
				frecverr(
				"request creation failed (cf: %lu bytes)", cfile_in);
				/*NOTREACHED*/
			    }
			    fname = cvt_spoolfn(ctf_hdr, CFILE);
			    print_request(rqp, fname);	   /* does putrequest(), etc. */
...
#endif
{
	...
	*reqid = '-';			/* restore file name */
	...
	rqp->user = Strdup(strrchr(user, '!') + 1);
	...
	getmessage(Msg, R_PRINT_REQUEST, &status, &reqid, &chkbits, &unused);
	if (status != MOK)
	...
	case MNOMEM:
		...
		if (chkbits) {
			if (chkbits & PCK_TYPE)
                		smail(user, E_LP_PGONE, NOLOG, Printer);
			else {
	    			char buf[20];
			...
			smail(user, E_LP_DENYDEST, NOLOG, Printer);
		break;
    	case MNOFILTER:
		smail(user, E_LP_NOFILTER, NOLOG);
		break;
    	case MERRDEST:
		smail(user, E_LP_REQDENY, NOLOG, Printer);
		break;
   	case MNOOPEN:
