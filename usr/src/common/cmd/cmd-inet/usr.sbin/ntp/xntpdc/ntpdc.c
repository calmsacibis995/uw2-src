...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/ntp/xntpdc/ntpdc.c	1.2"
#ident	"$Header: $"
 ...
 */
void help(), timeout(), delay(), host(), Poll(), passwd(), hostnames();
void setdebug(), quit(), keyid(), version();
	...
	int auth;
{
	...
	if (!auth) {
		...
		if (auth_havekey(info_auth_keyid)) {
			...
			auth2crypt(info_auth_keyid, (u_long *)&qpkt,
			    REQ_LEN_NOMAC);
	...
	char *cmdline;
{
	...
	if (interactive) {
		if (!setjmp(interrupt_buf)) {
			oldintr = sigset(SIGINT, abortcmd);
			(xcmd->handler)(&pcmd, current_output);
		}
		(void) sigset(SIGINT, oldintr);
	} else {
		(xcmd->handler)(&pcmd, current_output);
	}
