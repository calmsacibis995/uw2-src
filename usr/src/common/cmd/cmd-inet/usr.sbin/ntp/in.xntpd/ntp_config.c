...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/ntp/in.xntpd/ntp_config.c	1.2"
#ident	"$Header: $"
	...
	char *argv[];
{
	...
	extern char *ntoa();
	static void save_resolve();
	static void do_resolve();
	static void abort_resolve();
	...
	u_long keyid;
{
	...
	(void) fprintf(res_fp, "%s %d %d %d %lu\n", name, mode,
	    version, flags, keyid);
}
