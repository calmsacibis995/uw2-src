...
#ident	"@(#)winxksh:xksh/gensym.sh	1.3"
...
list() {
	PATH=:$PATH:../xksh
	sh listsyms $i | grep '^[A-Za-z0-9_]\{1,19\}$' | grep -v fptrap | sort -u
}
tmp=/tmp/X$$
found=
targ=$1
...
do
	...
	excl_lisi=`trunc $basei.exc`
	pre_lisi=`trunc $basei.pre`
	FILES="$FILES $lisi"
	rm -f "$tmp"
	...
	then
		...
		then
			...
			if [ -f "$lisi" -a -f "$pre_lisi" ] && diff "$pre_lisi" "$tmp" > /dev/null
			then
				:
			else
				...
				if [ -f "$excl_lisi" ]
				then
					diff "$tmp" "$excl_lisi" | sed -n 's/< //p' > "$lisi"
					mv -f "$tmp" "$pre_lisi"
				else
					mv -f "$tmp" "$lisi"
				fi
