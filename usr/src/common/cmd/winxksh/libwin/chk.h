...
#ident	"@(#)winxksh:libwin/chk.h	1.4"
#ifndef	_CHK_H
#define	_CHK_H
...
typedef struct {
	...
	int (*on_cb)(int, void *); /* callback function for select */
	void *on_cb_parm;	/* Pointer to args for select callback */
	int (*off_cb)(int, void *); /* callback function for unselect */
	void *off_cb_parm;	/* Pointer to args for unselect callback */
} CHECK_ITEM;
...
typedef struct {
	...
	CHECK_ITEM **items;	/* Pointer to the radio box items */
	short count;
	short cur_row;
	short cur_col;
	short cur_vrow;
