...
#ident	"@(#)vi:port/ex_put.c	1.22.1.16"
...
register wchar_t c;
{
	...
	if ((int)(c & QUOTE))
		...
		putchar('^'), c = ctlof(c);
        else if (c >= 0200 && (putoctal || 
				(multibyte && !wisprint(c)) ||
				(!multibyte && iscntrl(c)) )) {
	/*
	 ...
	 * does not include 8-bit characters, we must make the additional
	 * checks (multibyte && !wisprint(c)) ||(!multibyte && iscntrl(c)) )
	 * instead of the simple !wisprint(c).  We count on the font server 
	 * to handle 8-bit characters that are not control characters.
	 *
	 * Should the C locale be modified to include 8-bit characters, these
	 * additional checks would become unnecessary.
	 */
		outchar('\\');
		outchar(((c >> 6) & 07) + '0');
		outchar(((c >> 3) & 07) + '0');
