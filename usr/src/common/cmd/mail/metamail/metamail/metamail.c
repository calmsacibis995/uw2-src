...
#ident	"@(#)mail:common/cmd/mail/metamail/metamail/metamail.c	1.1.3.4"
	...
	if (MightAskBeforeExecuting
		...
		switch(ans) {
			...
			while (needname) {
				...
				do {
				    pfmt(stderr, MM_NOSTD, ":145:File %s exists.  Do you want to overwrite it (%s/%s) ?\n", Fname, yesstr, nostr);
				    s = gets(AnsBuf);
				    if (!s) {
					overwriteans = 0;
				    } else {
					...
					while (*s && isspace((unsigned char) *s)) ++s;
					len = strlen(s);
					if ((len > 0) && strncmp(s, yesstr, len) != 0)
					    overwriteans = 1;
					else if ((len == 0) || ((len > 0) && strncmp(s, nostr, len) != 0))
					    overwriteans=0;
				    }
				    if (overwriteans == -1) pfmt(stderr, MM_ERROR, ":146:Please answer %s or %s.\n", yesstr, nostr);
...
char *ctype, *progname, *label;
{
	...
	printf("\n");
	if (label) {
	    pfmt(stderr, MM_NOSTD, ":189:This message contains %s.\nDo you want to view it using the '%s' command (%s/%s) [%s] ? ", label, ShortCommand(progname), yesstr, nostr, yesstr);
	} else {
	    pfmt(stderr, MM_NOSTD, ":190:This message contains '%s'-format data.\nDo you want to view it using the '%s' command (%s/%s) [%s] ? ", ctype, ShortCommand(progname), yesstr, nostr, yesstr);
	}
	s = gets(AnsBuf);
	if (!s) return(0); /* EOF */
	while (*s && isspace((unsigned char) *s)) ++s;
	len = strlen(s);
	if (len == 0) return(1);
	if (strncmp(s, yesstr, len) == 0) return(1);
	if (strncmp(s, nostr, len) == 0) return(0);
	pfmt(stderr, MM_ERROR, "Please answer %s or %s.\n", yesstr, nostr);
    }
}
