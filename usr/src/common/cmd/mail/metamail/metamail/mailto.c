...
#ident	"@(#)mail:common/cmd/mail/metamail/metamail/mailto.c	1.1.1.5"
...
char **argv;
{
	...
	while (!AllDone && fgets(LineBuf, MAX_LINELENGTH, stdin) != NULL) {
		...
		switch (LineBuf[1]) {
			...
			break;
		    case '!':
			if (!V_quiet) pfmt(stdout, MM_INFO, "Executing: %s\n", start);
			SYSTEM(start);
			break;
		    case '>':
...
CreateNewPart() {
	...
	if (ans == 1) {
		...
		if (index(LineBuf, '/')) {
		    ...
		    pfmt(stderr, MM_NOSTD, ":109:The MIME content-type '%s' is not listed in your local mailcap files,\nand may not be a valid MIME type.  Do you want to use it anyway [%s] ? ", LineBuf, nostr);
		    s = gets(AnsBuf);
		    while (s && *s && isspace((unsigned char) *s)) ++s;
		    if (s && ((len = strlen(s)) > 0) && strncmp(s, yesstr, len) == 0) break;
		    continue;
		}
