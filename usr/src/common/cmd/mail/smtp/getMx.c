...
#ident	"@(#)mail:common/cmd/mail/smtp/getMx.c	1.2"
...
void
    getMxRecords(msgConn_t *msgConn_p, char *systemName, char *mySystemName)
	{
	mxRecord_t
	    *curRec_p;
	...
	if(DebugLevel > 4)
		...
		(
		stderr,
		"getMxRecords(0x%x, %s, %s) Entered.\n",
		(int) msgConn_p,
		systemName,
		(mySystemName == NULL)? "NIL": mySystemName
		);
	    }
	...
	strncpy(nameBuffer, systemName, sizeof(nameBuffer) - 2);
	strcat(nameBuffer, ".");
	SearchCount++;
	if((list = linkNew(NULL)) == NULL)
	    {
	    }
	else if
	    (
	    ...
	    while((curRec_p = (mxRecord_t *)linkOwner(linkNext(list))) != NULL)
		{
		if(localPreference < curRec_p->mx_preference)
		    {
		    msgConnAddHost(msgConn_p, curRec_p->mx_systemName);
		    }
	...
	if(DebugLevel > 4) (void) fprintf(stderr, "getMxRecords() Exited.\n");
	}
