...
#ident	"@(#)mail:common/cmd/mail/pathrouter/resolve.c	1.2"
...
#ident "@(#)resolve.c	1.2 'attmail mail(1) command'"
/*
**
**  Resolve.c
**
...
**
*/
/* static char 	*sccsid="@(#)resolve.c	2.6 (smail) 5/24/88"; */
...
static int route ARGS(( char *domain, char *user, int look_smart, char *result, int *cost));
...
/* For use as SVR4 mail router - should skip over leading !'s
 * that are used by the mailsurr.
 */
	if (*address == '!') /* skip over leading !'s */
		address++;
...
*/
	if((cascmp(domain, hostname) == 0)
	|| (cascmp(domain, hostdomain) == 0)
	|| (cascmp(domain, localhost) == 0)) {
		step = 0;
		*cost = 0;
		(void) strcpy(path, "%s");
...
*/
        if ((step != 0) && domainv[step] && ((strcmp(path,"%s")) == 0)) {
	    /*
	    ** part of the right-hand side of the entire domain name has
	    ** been matched. This portion needs to be stripped out, and
	    ** the remainder of the left-hand side needs to be used in
	    ** building up the address to which the mail will be delivered.
	    **
	    ** The right-hand portion to be stripped off is contained in
	    ** domainv[step]-1.
	    */
	    for (tdom = domain; strcmp(tdom,domainv[step]-1) != 0; tdom++);
