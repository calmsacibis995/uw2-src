...
#ident	"@(#)mail:common/cmd/mail/re/egcomp.c	1.2.4.3"
...
#ident "@(#)egcomp.c	1.6 'attmail mail(1) command'"
#include	<libc.h>
#include	"re.h"
#include	"lre.h"
...
dummy()
{
	return(eg_newexpr(Dot, '.', (Expr *)0, (Expr *)0));
}
	...
	unsigned char *map;
{
	...
	if(setjmp(gohome) == 0){
		...
		e = 0;
/*{char buf1[4096]; eg_epr(e, buf1, 0); printf("e='%s'\n", buf1);}/**/
	return(e);
}
