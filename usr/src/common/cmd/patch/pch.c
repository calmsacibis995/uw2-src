...
#ident	"@(#)patch_p2:pch.c	1.1"

/*
 * OSF/1 1.2
 */
#if !defined(lint) && !defined(_NOIDENT)
static char rcsid[] = "@(#)$RCSfile: pch.c,v $ $Revision: 1.1.2.2 $ (OSF) $Date: 1992/07/06 18:33:01 $";
#endif
...
grow_hunkmax()
{
	...
	return;
    if (!using_plan_a)
	fatal1(":45: out of memory (grow_hunkmax)\n");
...
there_is_another_patch()
{
	...
	if (p_base != 0L) {
		...
		pfmt(stderr, MM_NOSTD,
			":48: Ignoring the trailing garbage.\ndone\n");
	}
	else
	    pfmt(stderr, MM_NOSTD,
...
":73:(Fascinating--this is really a new-style context diff but without\n");
		pfmt(stderr, MM_NOSTD, 
":102:the telltale extra asterisks on the *** line that usually indicate\n");
		pfmt(stderr, MM_NOSTD, ":103:the new style...)\n");
	    }
