...
#endif
{
	...
	while ((cur = newline(fd)) != 0)
	{
		...
		if (ans != want)
			continue;
		if (nmat != 0)	/* OPT_ENTIRE: check for whole line */
		{
...
static const char usestr[] = ":1231:Usage:\n\
\t[-E|-F] [-c|-l|-q] [-bhinsvx] pattern [file ...]\n\
\t[-E|-F] [-c|-l|-q] [-bhinsvx] -e pattern ... [-f file ...] [file ...]\n\
\t[-E|-F] [-c|-l|-q] [-bhinsvx] -f file ... [-e pattern ...] [file ...]\n";
...
#endif
{
	...
	*/
	if ((p = strrchr(argv[0], '/')) != 0)
		...
		p = &argv[0][0];
	regflags = REG_OLDBRE & ~REG_ESCNL;	/* POSIX.2 allows */
	if (*p == 'e')
	{
	...
	* Gather options.
	*/
	while ((ch = getopt(argc, argv, "EFbce:f:hilnqsvxy")) >= 0)
	{
		switch (ch)
		{
		usage:;
