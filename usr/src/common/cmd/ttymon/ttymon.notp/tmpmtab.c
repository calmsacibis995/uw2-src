...
#ident	"@(#)ttymon:common/cmd/ttymon/ttymon.notp/tmpmtab.c	1.3"
		...
		*ptr = '\0';
		...
		for (state=P_TAG,ptr=line;state !=FAILURE && state !=SUCCESS;) {
			...
			case P_DMSG:
				...
				break;
			case P_SAKTYPE:
				gptr->p_saktype = strsave(getword(ptr,&size,0));
				break;
			case P_SAKDEF:
				gptr->p_sakdef = strsave(getword(ptr,&size,0));
				break;
			case P_SAKSEC:
				gptr->p_saksec = strsave(getword(ptr,&size,0));
				break;
			} /* end switch */
			ptr += size;
			if (state == FAILURE) 
...
dump_pmtab()
{
	...
	for (gptr=PMtab; gptr; gptr = gptr->p_next) {
		...
		log(Scratch);
		(void)sprintf(Scratch,"iascheme:\t%s",gptr->p_iascheme);
		log(Scratch);
		...
		log(Scratch);
		(void)sprintf(Scratch,"sak type:\t%s",gptr->p_saktype);
		log(Scratch);
		(void)sprintf(Scratch,"sak define:\t%s",gptr->p_sakdef);
		log(Scratch);
		(void)sprintf(Scratch,"secondary sak:\t%s",gptr->p_saktype);
		log(Scratch);
...
struct	pmtab	*e1,*e2;
{
	...
	if (strcmp(e1->p_dmsg, e2->p_dmsg) != 0)
		return(0);
	if (strcmp(e1->p_saktype, e2->p_saktype) != 0)
		return(0);
	if (strcmp(e1->p_sakdef, e2->p_sakdef) != 0)
		return(0);
	if (strcmp(e1->p_saksec, e2->p_saksec) != 0)
		return(0);
...
# endif
					...
					sp->p_pid = tsp->p_pid;
				        sp->p_inservice =
						tsp->p_inservice;
					sp->p_ignoretime = tsp->p_ignoretime;
					sp = tsp;
				}
				Nentries++;
			}
...
struct	pmtab	*p;
{
	...
	if ((p->p_identity == NULL) || (*(p->p_identity) == '\0')) {
                if ((p->p_iascheme == NULL) || (*(p->p_iascheme) == '\0')) {
                        log("identity and scheme fields are missing");
                        return(-1);
                }
                else {
                        p->p_uid = 0;
