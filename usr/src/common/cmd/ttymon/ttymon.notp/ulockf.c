...
#ident	"@(#)ttymon:common/cmd/ttymon/ttymon.notp/ulockf.c	1.3"
...
/*
 * fn_getpid(name) - Get the PID of the process that holds the lock on the
 *                   device
 *		     NOTE: this is the same code as cklock, except it returns
 *		     the PID
 *
 * return -
 *	SUCCESS - return the PID of the process that has dev locked.
 *	FAIL - return -1
 */
...
/*
 * remove the lock file associated with the device name
 * note: this is used when we don't have an FD and there is not one open
 * by this process. this is important since we will not do the advisory lock 
 * here like in fd_rmlock
 *
 * return -
 *	SUCCESS - BNU lock file  removed
 *	FAIL - 
 */
...
char *name;
{
    ...
    (void) sprintf(devname, "%s%s", (*name == '/' ? "" : "/dev/"), name);
	
    if ( stat(devname, &_st_buf) == 0 ) {
        (void) sprintf(lockname, "%s.%3.3lu.%3.3lu.%3.3lu", L_LOCK,
            (unsigned long) major(_st_buf.st_dev),
	    (unsigned long) major(_st_buf.st_rdev),
	    (unsigned long) minor(_st_buf.st_rdev));
