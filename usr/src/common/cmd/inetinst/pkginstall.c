...
#ident	"@(#)inetinst:pkginstall.c	1.5"

/*
 *  pkginstall.c
 *
 *  Client side of network install.  Request software from a server
 *  and installs from the datastream received.
 */
...
main(int argc, char *argv[])
{
	...
	char	*target;		/* Target specification for list */
        char    *network;               /* Network type (tcp/spx) */
	char	*src_host;		/* Host part of source */
	char	*src_device;		/* Device part of source */
	char	*trg_host;		/* Host part of source */
	char	*trg_device;		/* Device part of source */
	...
	int	sflag=0;		/* Source option flag */
	int	tflag=0;		/* Target option flag */
	int	Nflag=0;		/* Noninteractive option flag */
	int	nflag=0;		/* Network option tracking flag */

	vflag=0;		/* Verbose option flag */
	...
	opterr = 0;
	while((c = getopt(argc, argv, "vNn:t:s:")) != -1) {
		switch (c) {
			case '?':
				usage(argv[0]);
				break;
	 ...
	 *  If no source or target were specified, assume local.
	 */
	if (sflag > 0) {
	 ...
	 */
	sprintf(netbuf, "%s %s\n", IOPT_PACKAGE_NAME, package);
	netsendrcv(netbuf, vflag);

	/*
	 *  Send OPTION 'interactive'; get response
	 */
	if (Nflag)
		sprintf(netbuf, "%s NO\n", IOPT_INTERACTIVE_NAME);
	else
		sprintf(netbuf, "%s YES\n", IOPT_INTERACTIVE_NAME);
	netsendrcv(netbuf, vflag);

	/*
	 *  Send OPTION 'source'; get response
	 */
	sprintf(netbuf, "%s %s:%s\n", IOPT_SOURCE_NAME, src_host, src_device);
	netsendrcv(netbuf, vflag);

	/*
	 *  Send OPTION 'target'; get response
	 */
	sprintf(netbuf, "%s %s:%s\n", IOPT_TARGET_NAME, trg_host, trg_device);
	netsendrcv(netbuf, vflag);
	...
	/*
	 *  If this machine is the target, we do the pkgadd
	 *  right from stdin.
	...
	if (!strcmp(get_nodename(), trg_host)) {
		if (!Nflag) {
			...
			sprintf(cmdbuf,"%s -d - %s >/dev/tty 2>/dev/tty",
			  PKGADD, package);
		} else {
			sprintf(logbuf,"%s -n -d - %s\n", PKGADD, package);
			log(logbuf);
		
			sprintf(cmdbuf,"%s -n -d - %s", PKGADD, package);
		}
			system(cmdbuf);
	} else {
...
usage(char *progname)
{
	...
	sprintf(logbuf, ":%d:%s\n", C_USAGE_PKGINSTALL, M_USAGE_PKGINSTALL);
