...
#ident	"@(#)rpcgen:rpc_main.c	1.8.8.5"
...
struct commandline *cmd;
{
	...
	if (allfiles){
		...
		(void) strncat(mkfilename, cmd->infile, (temp - cmd->infile));
	} else 
	...
	f_print(fout, "OBJECTS_CLNT = $(SOURCES_CLNT.c:%%.c=%%.o) $(TARGETS_CLNT.c:%%.c=%%.o) ");
...
usage()
{
	...
	f_print(stderr,
		"\t%s [-a][-b][-C] [-Dname[=value]] [-i size]  [-I [-K seconds]] [-L][-N][-T] [-Y path] infile\n",
			cmdname);
	f_print(stderr,
