...
register char *name;
{
	...
	if (onelock(pid, tempfile, name) == -1) {
		...
		if (cklock(name))
			...
			(void) unlink(tempfile);
			DEBUG(4,gettxt(":335", "ulockf failed in onelock()\n%s"), "");
			return(FAIL);
		    }
		}
	}
...
register char *name;
{
	...
	DEBUG(4, gettxt(":120", "cklock: checking lock file %s\n"), name);
	if (fd == -1) {
	    if (errno == ENOENT)  /* file does not exist -- OK */
		return(0);
	    DEBUG(4,gettxt(":121", "cklock: cannot read lock file (errno=%d) -- removing it!\n"), errno);
	    goto unlk;
	}
	...
	if (ret != (SIZEOFPID+1)) {
	    ...
	    DEBUG(4, gettxt(":122", "cklock lock file is corrupted -- removing it!\n"), "");
	    goto unlk;
	}
	lpid = (pid_t) strtol(alpid, (char **) NULL, 10);
	if ((ret=kill(lpid, 0)) == 0 || errno == EPERM) {
	    DEBUG(4, gettxt(":123", "cklock: process using lock file is still active -- lock file not removed\n"), "");
	    return(FAIL);
	}
	else { /* process no longer active */
	    DEBUG(4, gettxt(":124", "cklock: kill(pid=%ld), "), (long) lpid);
	    DEBUG(4, gettxt(":125", " returned %d"), ret);
	    DEBUG(4, gettxt(":126", "--removing lock file (%s)\n"), name);
	}
unlk:
	
	if (unlink(name) != 0) {
		DEBUG(4,gettxt(":127", "cklock: unlink(%s) failed\n"), name);
		return(FAIL);
	}
	return(0);
}
...
int fd;
{
	...
	DEBUG(7, gettxt(":337", "fd_mklock: lockf() failed, errno=%d\n"), errno);
    ...
    DEBUG(7, gettxt(":338", "fd_mklock: exiting ok\n%s"), "");
    return(SUCCESS);
}
