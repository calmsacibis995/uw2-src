...
#ident	"@(#)cs:cs/sysfiles.c	1.1.2.9"
#ident	"$Header: $"
...
int type;
{
	...
	default:
		(void)sprintf(errformat, gettxt(":334", "bad access type %d"), type);
	...
	for (line = tokens; (line - tokens) < NTOKENS && *line; line++) {
		...
		*equals = '\0';
		...
		if (strcmp(*line, "systems") == SAME)
			...
			DEBUG(5, gettxt(":106", "unrecognized label <%s> in file Sysfiles or Devconfig\n"),*line);
		}
	}
	return;
}
...
int fd;
{
	...
	DEBUG(5, (optional ? gettxt(":107", "pop_push: optionally popping STREAMS module %s\n")
			   : gettxt(":108", "pop_push: popping STREAMS module %s\n") ), strmod);
	if (!isastream(fd)){
	    DEBUG(5, gettxt(":109", "pop_push: cannot pop or push modules on a non-STREAMS device\n"),"");
	    return(FALSE);
	}
	if ( ioctl(fd, I_LOOK, onstream) == -1 ) {
	    if ( errno == EINVAL && optional ) {
		continue;
	    } else {
		DEBUG(5, gettxt(":110", "pop_push: ioctl(I_LOOK) on fd %d failed"), fd);
		DEBUG(5, ", errno=%d\n", errno);
		return(FALSE);
	    }
	}
	if ( strcmp(strmod, onstream) != SAME ) {
	    if ( optional )
		continue;
	    DEBUG(5, gettxt(":111", "pop_push: module <%s> not on stream\n"), strmod);
	    return(FALSE);
	}
	if ( ioctl(fd, I_POP, 0) == -1 ) {
	    DEBUG(5, gettxt(":112", "pop_push: ioctl(I_POP) on fd %d failed"), fd);
	    DEBUG(5, ", errno=%d\n", errno);
	    return(FALSE);
	}
    }
	...
	DEBUG(5, gettxt(":113", "pop_push: ioctl(I_LIST) on fd %d failed"), fd);
	DEBUG(5, ", errno=%d\n", errno);
	return(FALSE);
    }
	...
	DEBUG(5, gettxt(":5", "memory allocation error in cs daemon, errno=%d"), errno);
	return(FALSE);
    }
	...
	for (i=0; i < mod_list.sl_nmods; i++) {
		if (strncmp(strmod, mod_list.sl_modlist[i].l_name,FMNAMESZ) == 0) {
			DEBUG(5,gettxt(":114", "module %s already pushed on stream\n"),strmod);
			break;
		}
	}

	if (i == mod_list.sl_nmods) {
		DEBUG(5, gettxt(":115", "pop_push: pushing STREAMS module %s\n"), strmod);
		if ( ioctl(fd, I_PUSH, strmod) == -1 ) {
	    		DEBUG(5, gettxt(":116", "pop_push: ioctl(I_PUSH) on fd %d failed"), fd);
	    		DEBUG(5, ", errno=%d\n", errno);
	    		return(FALSE);
		}	
	}
...
setconfig()
{
	...
	while (getline(f, buf) > 0) { 
		...
		if (strncmp("Protocol=", tok, strlen("Protocol=")) == SAME) {
		    ...
		    if ( *tok != '\0' ) {
			if ( _ProtoCfg[0] != '\0' ) {
			    DEBUG(7, gettxt(":117", "\'Protocol\' string <%s> "), tok);
			    DEBUG(7, gettxt(":118", " overrides <%s>\n"), _ProtoCfg);
		        }
		        strcpy(_ProtoCfg, tok);
		    }
	        } else {
		    DEBUG(7, gettxt(":119", "Unknown configuration parameter %s\n"), tok);
	        }
	    }
	}
    }
