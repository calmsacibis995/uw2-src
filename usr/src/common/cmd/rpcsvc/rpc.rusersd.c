...
#ident	"@(#)rpcsvc:rpc.rusersd.c	1.12.7.5"
	...
	register SVCXPRT *transp;
{
	...
	case RUSERSPROC_ALLNAMES:
		if (rqstp->rq_vers == RUSERSVERS_ORIG) {
			...
			utmparr.uta_cnt = cnt;
			if (!svc_sendreply(transp, xdr_utmparr, (caddr_t)&utmparr))
				perror("svc_rpc_send_results");
			svc_freeargs(transp, xdr_utmparr, (caddr_t)&utmparr);
			free(utmparr.uta_arr);
		} else {
			...
			if (!svc_sendreply(transp, xdr_utmpidlearr,
					(caddr_t)&utmpidlearr))
				perror("svc_rpc_send_results");
			svc_freeargs(transp, xdr_utmpidlearr,
					(caddr_t)&utmpidlearr);
