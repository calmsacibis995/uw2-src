...
#ident	"@(#)ucb:common/ucblib/libc/port/gen/_times_power.c	1.1"
#ident	"$Header: $"
...

{
	...
	if (n > 0) {		/* The tables aren't big enough to accomodate
		...
		*pnewbf = (_big_float *)BIG_FLOAT_TIMES_TOOBIG;
		goto ret;
	}
			...
			if (lengthp == 1) {	/* Special case - multiply by
				...
				for (j = lengthx + lengthp - 2; j >= 0; j--) {
					...
					if (lengthprod > needed_precision)
						...
						excess = 0;
					if ((i == itlast) && ((j + 2) <= (int)excess) && (pbf->bsignificand[j + 1] <= canquit)
