...
#ident	"@(#)xcpxinstall:common/xcpcmd/xinstall/customR4.sh	1.1.1.9"
...
FLIST=/tmp/cusfl$$			# list of names of files 
					# installed by tar
...
cleanup() {
	trap '' 1 2 3 15
	rm -f $init $prep $tmp* $ser $rmv_perm $FLIST
	exit $1
}
...
un_compress() {
	...
	do
		[ -s $filename ] || continue
		file $filename | grep "compressed data" >/dev/null || continue
...
if [ "$cmdname" -a $logflg = "y" ]
then
	cmdflg='setlogtrap; $cmdname $arg; logit'
elif [ "$cmdname" -a $logflg = "n" ]
then
	cmdflg='"$cmdname" $arg'
fi
