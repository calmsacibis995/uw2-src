...
/*manpage*NWNCP23s105CloseFileStartQJob*************************************
...
REMARKS: This call closes a client's queue job and marks it ready for execution as
         listed below.  The Job Number field contains the number that the queue
         management process assigned to the job when the job first entered the queue.

         - The queue file associated with the job is closed.

         - The Entry Open flag (in the Job Control Flags) is reset.

         - The entry is considered ready for service if the User Hold flag
         and the Operator Hold flag are both clear.
...
)
{
   ...
   #define NCP_SUBFUNCTION ((nuint8) 105)
...
/*
$Header: /SRCS/esmp/usr/src/nw/lib/libnwNcp/23s105.c,v 1.7 1994/09/26 17:34:51 rebekah Exp $
*/
