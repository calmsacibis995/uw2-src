...
#ident	"@(#)libc-port:re/regdfa.c	1.6"
...
typedef struct
{
	...
	wint_t	op;	/* the leaf match operation */
} Posn;
...
#else
copy(ep, tp)regex_t *ep; Tree *tp;
#endif
{
	wint_t wc;
	Tree *np;

	if ((np = (Tree *)malloc(sizeof(Tree))) == 0)
		return 0;
...
static Tree *
#ifdef __STDC__
findposn(regex_t *ep, Tree *tp)
#else
findposn(ep, tp)regex_t *ep; Tree *tp;
#endif
{
	unsigned int lo, hi;
	Tree *ptr, *par;
	...
	default:
		if (ep->re_flags & REG_ICASE
			&& (wc = towupper(tp->op)) != tp->op)
		{
			if ((ptr = _reg1tree(tp->op, (Tree *)0)) == 0)
				return 0;
			ptr->parent = tp;
			ptr->left.pos = ep->re_dfa->nposn++;
			tp->op = ROP_OR;
			tp->left.ptr = ptr;
			ptr = _reg1tree(wc, (Tree *)0);
	...
	case ROP_OR:
	case ROP_CAT:
		if ((tp->right.ptr = findposn(ep, tp->right.ptr)) == 0)
			return 0;
		/*FALLTHROUGH*/
	case ROP_STAR:
	case ROP_PLUS:
	case ROP_QUEST:
	case ROP_LP:
		if ((tp->left.ptr = findposn(ep, tp->left.ptr)) == 0)
			return 0;
		return tp;
	case ROP_BRACE:
		if ((tp->left.ptr = findposn(ep, tp->left.ptr)) == 0)
	...
	*/
	...
	if (lo == 0)
	{
		if ((tp->left.ptr = _reg1tree(ROP_QUEST, tp->left.ptr)) == 0)
			return 0;
	...
	else
	{
		...
		while (--lo != 0)
		{
			if ((tp = _reg2tree(ROP_CAT, tp, copy(ep, ptr))) == 0)
				return 0;
		}
	}
	if (hi == BRACE_INF)
	{
		if ((tp->right.ptr = _reg1tree(ROP_PLUS, tp->right.ptr)) == 0)
			return 0;
		tp->right.ptr->parent = tp;
	}
	else if (hi != 0)
	{
		if ((tp->right.ptr = _reg1tree(ROP_QUEST, tp->right.ptr)) == 0)
			return 0;
		ptr = tp->right.ptr;
		ptr->parent = tp;
		while (--hi != 0)
...
skip:;
	p->op = tp->op;
	memset((void *)dp->posset, 0, dp->nposn);
	dp->nset = 0;
	follow(dp, tp);
	...
	if ((p->nset = dp->nset) > dp->avail) /* need more */
	{
		if ((n = p->nset << 1) < dp->nposn)
			n = dp->nposn;	
		dp->avail += n;
		if ((fp = (size_t *)realloc((void *)dp->posfoll,
...
#endif
{
	...
	*/
	...
	{
		i = dp->avail + n << 1;
		if ((fp = (size_t *)realloc((void *)fp,
			sizeof(size_t) * (i + dp->used))) == 0)
		{
			return 0;
		}
		dp->avail = i;
...
_regdeldfa(Dfa *dp)
#else
_regdeldfa(dp)Dfa *dp;
#endif
{
	if (dp->posfoll != 0)
		free((void *)dp->posfoll);
	if (dp->sigfoll != 0)
		free((void *)dp->sigfoll);
	if (dp->cursig != 0)
		free((void *)dp->cursig);
	if (dp->posn != 0)
		free((void *)dp->posn);
...
static int
#ifdef __STDC__
trans(Dfa *dp, int st, wint_t wc)
#else
trans(dp, st, wc)Dfa *dp; int st; wint_t wc;
#endif
{
	...
	do
	{
		...
		case ROP_BKT:
		case ROP_BKTCOPY:
			if (wc > '\0' && _bktmbexec(pp->bkt, wc, s) == 0)
				break;
			continue;
		}
		/*
...
int
#ifdef __STDC__
_regdfacomp(regex_t *ep, Tree *tp, Lex *lxp)
#else
_regdfacomp(ep, tp, lxp)regex_t *ep; Tree *tp; Lex *lxp;
#endif
{
	Tree *lp;
	Dfa *dp;
	...
	* "any match" _regdfaexec() not a special case, and the
	* initial state signature will fall out when building
	* the follow sets for all the leaves.
	*/
	if ((lp = _reg1tree(ROP_ALL, (Tree *)0)) == 0
		|| (lp = _reg1tree(ROP_STAR, lp)) == 0
		|| (tp->left.ptr = lp
			= _reg2tree(ROP_CAT, lp, tp->left.ptr)) == 0)
	{
		return REG_ESPACE;
	}
	lp->parent = tp;
	if ((dp = (Dfa *)calloc((size_t)1, sizeof(Dfa))) == 0)
		return REG_ESPACE;
	ep->re_dfa = dp;
	/*
	...
	*/
	dp->avail += dp->used;
	dp->used = 0;
	if ((dp->sigfoll = (size_t *)malloc(sizeof(size_t) * dp->avail)) == 0)
		goto err;
	...
	if ((dp->cursig = (size_t *)malloc(sizeof(size_t) * dp->nposn)) == 0)
		goto err;
	dp->nfix = 2;
	if ((st = trans(dp, 1, (wint_t)ROP_BOL)) == 0)
		goto err;
...
#endif
{
	...
	for (;;)
	{
		...
		else if (!ISONEBYTE(wc) && (i = _mb2wc(&wc, s)) > 0)
			s += i;
		if ((wc & ~(long)(NCHAR - 1)) != 0
			|| (nst = dp->trans[st][wc]) == 0)
		{
			if ((nst = trans(dp, st, wc)) == 0)
				return REG_ESPACE;
		...
		if ((st = nst - 1) == 0) /* dead state */
		{
			...
			if ((wc = *beg++) == '\0')
				return REG_NOMATCH;
			else if (!ISONEBYTE(wc) && (i = _mb2wc(&wc, beg)) > 0)
				beg += i;
			s = beg;
			st = dp->leftmost;
			goto newst;
...
int
#ifdef __STDC__
_regdfaexec(Dfa *dp, Exec *xp)
#else
_regdfaexec(dp, xp)Dfa *dp; Exec *xp;
#endif
{
	const unsigned char *s;
	int i, nst, st;
	...
	dp->flags = xp->flags & REG_NOTEOL;	/* for trans() */
	if (xp->nmatch != 0)
