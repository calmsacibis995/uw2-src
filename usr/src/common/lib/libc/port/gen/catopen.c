...
#ident	"@(#)libc-port:gen/catopen.c	1.10"
  ...
  int mode;   		
{
  ...
  if (mode==NL_CAT_LOCALE) {
	if ((lang = setlocale(LC_MESSAGES, NULL)) == 0)
    		lang = NL_DEF_LANG;
  } else if ((lang = getenv(NL_LANG)) == 0)
    		lang = NL_DEF_LANG;
  ...
  int mode;
{
   ...
   */
  if ((name = cat_name((char *)name,path,mode)) != 0){
