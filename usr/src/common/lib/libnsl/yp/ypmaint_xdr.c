...
#ident	"@(#)libnsl:common/lib/libnsl/yp/ypmaint_xdr.c	1.3.7.2"
#ident  "$Header: $"
	...
	char *pstring;
{
	...
	if (xdrs->x_op == XDR_FREE) {
		/* All storage for ypmaplist is allocated by xdr_reference() */
		return (TRUE);
	}
	...
	struct netconfig *objp;
{
	...
	if (!xdr_vector(xdrs, (char *)objp->nc_unused, 8, sizeof(u_long),
	    xdr_u_long)) {
		return (FALSE);
	}
	return (TRUE);
