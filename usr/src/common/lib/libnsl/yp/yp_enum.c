...
#ident	"@(#)libnsl:common/lib/libnsl/yp/yp_enum.c	1.2.7.2"
#ident  "$Header: $"
	...
	int  *vallen;		/* return: bytes in val */
{
	...
	for (;;) {
		...
		if (pdomb->dom_binding->ypbind_hi_vers == YPVERS) {
			/*
			 * lock _yp_domain_entry_lock to use handle.
			 */
...

{
	...
	if(clnt_call(pdomb->dom_client, YPPROC_FIRST,
			...
			(xdrproc_t)xdr_ypresp_key_val, (caddr_t)&resp,
			timeout) != RPC_SUCCESS) {
		return (YPERR_RPC);
	}
	...
	if (!retval) {
		if ((*key = (char *) malloc((unsigned)resp.keydat.dsize + 2))
					!= NULL) {
			if ((*val = (char *) malloc(
	...
	for (;;) {
		...
		if (pdomb->dom_binding->ypbind_hi_vers == YPVERS) {
			...
			reason = donext(domain, map, inkey, inkeylen, pdomb,
					_ypserv_timeout, outkey, outkeylen,
					val, vallen);
		        MUTEX_UNLOCK (&pdomb->_yp_domain_entry_lock);
		        RW_UNLOCK(&_yp_domain_list_lock);
...

{
	...
	if(clnt_call(pdomb->dom_client, YPPROC_NEXT,
