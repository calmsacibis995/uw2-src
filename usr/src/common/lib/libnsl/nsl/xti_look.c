...
#ident	"@(#)libnsl:common/lib/libnsl/nsl/xti_look.c	1.1"
#ident	"$Header: $"
...
int fd;
{
	...
	/* Call the auxiliary function __xti_look() whose code was removed
	 * from _xti_look() so that it could be called by other TLI functions
	 * that already held tiptr->lock.
	 */
	retval = __xti_look(fd, tiptr);
...
/*
 * This function implements the XTI semantics.
 */
__xti_look(fd, tiptr)
int fd;
struct _ti_user *tiptr;
{
	...
	/*
         * If there is nothing there and the flow control flags have
         * been set by a previous TLI call, check to see if flow control
         * has been lifted. If so, clear the flags and return the T_GODATA
         * or T_GOEXDATA events.
         */
        if (tiptr->ti_flags & FLOWCNTL)
                if (ioctl(fd, I_CANPUT, TI_NORMAL))  {
                        tiptr->ti_flags &= ~FLOWCNTL;
                        return(T_GODATA);
                }

        if (tiptr->ti_flags & FLOWCNTLEX)
                if (ioctl(fd, I_CANPUT, TI_EXPEDITED)) {
