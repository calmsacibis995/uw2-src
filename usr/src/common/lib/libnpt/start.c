...
#ident	"@(#)libnpt:start.c	1.2"
static char sccsid[] = "@(#)$Header: /SRCS/esmp/usr/src/nw/lib/libnpt/start.c,v 1.1 1994/02/11 23:05:59 eric Exp $";
#endif
...
uint16
PSStartPrinter(connectID, printerNumber)
	...
	uint8		printerNumber;	/* Printer number					*/
{
	...
	dprintf("PSStartPrinter: id-%d pr-%d\n",
		connectID, printerNumber);

	memset(&request, 0, sizeof(NptRequest_t));

	request.FunctionNumber = CMD_START_PRINTER;
