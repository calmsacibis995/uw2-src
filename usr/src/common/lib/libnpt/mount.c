...
#ident	"@(#)libnpt:mount.c	1.2"
static char sccsid[] = "@(#)$Header: /SRCS/esmp/usr/src/nw/lib/libnpt/mount.c,v 1.1 1994/02/11 23:05:41 eric Exp $";
#endif
...
uint16
PSSetMountedForm(connectID, printerNumber, formNumber)
	uint16		connectID;		/* SPX Connection number			*/
	uint8		printerNumber;	/* Printer number					*/
	uint16		formNumber;		/* Form number						*/
{
	uint16			ccode;
	NptRequest_t	request;


	dprintf("PSSetMountedForm: id-%d pr-%d fm-%x\n",
		connectID, printerNumber, formNumber);

	memset(&request, 0, sizeof(NptRequest_t));

	request.FunctionNumber = CMD_MOUNT_FORM;
