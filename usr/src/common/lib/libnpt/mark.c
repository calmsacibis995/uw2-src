...
#ident	"@(#)libnpt:mark.c	1.2"
static char sccsid[] = "@(#)$Header: /SRCS/esmp/usr/src/nw/lib/libnpt/mark.c,v 1.1 1994/02/11 23:05:39 eric Exp $";
#endif
 ...
 *
 * Description:	This file contains MarkTopOfForm
 *
...
uint16
PSMarkTopOfForm(connectID, printerNumber, character)
	uint16		connectID;		/* SPX Connection number			*/
	uint8		printerNumber;	/* Printer number					*/
	char		character;		/* Character to mark form with		*/
{
	uint16			ccode;
	NptRequest_t	request;


	dprintf("PSMarkTopOfForm: id-%d prnt-%d ch-%x\n",
		connectID, printerNumber, character);

	memset(&request, 0, sizeof(NptRequest_t));

	request.FunctionNumber = CMD_MARK_PAGE;
