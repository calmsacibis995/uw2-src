...
#ident	"@(#)libnpt:stop.c	1.2"
static char sccsid[] = "@(#)$Header: /SRCS/esmp/usr/src/nw/lib/libnpt/stop.c,v 1.1 1994/02/11 23:06:01 eric Exp $";
#endif
...
uint16
PSStopPrinter(connectID, printerNumber, jobOutcome)
	...
	uint8		jobOutcome;		/* Job outcome						*/
{
	...
	dprintf("PSStopPrinter: id-%d pr-%d out-%x\n",
		connectID, printerNumber, jobOutcome);

	memset(&request, 0, sizeof(NptRequest_t));

	request.FunctionNumber = CMD_STOP_PRINTER;
