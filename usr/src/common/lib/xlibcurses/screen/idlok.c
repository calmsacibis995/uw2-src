...
#ident	"@(#)curses:common/lib/xlibcurses/screen/idlok.c	1.19.2.5"
...
int	topy, boty;
{
    ...
    this_sid = sid;
    idby = _MIN(this_sid->_wy, this_sid->_sy); /* an initial value */
    while (++this_sid < eid)
       idby = _MIN(idby, _MIN(this_sid->_wy,this_sid->_sy));
    return (eid > sid ? idby : scrli);
}
