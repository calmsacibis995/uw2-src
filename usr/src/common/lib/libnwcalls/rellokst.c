...
#ident	"@(#)libNwCalls:rellokst.c	1.5"
#include "nwcaldef.h"
#include "nwintern.h"
#include "nwundoc.h"
...
/*manpage*NWReleaseFileLockSet**********************************************
SYNTAX:  NWCCODE N_API NWReleaseFileLockSet
         (
            void
         );

REMARKS: Unlocks all files identified in the log table, but does not remove
         them from the table.
...
)
{
   ...
   reqBuf[0] = 0;  /* Release all locked sets */

   reqFrag[0].fragAddress = reqBuf;
   reqFrag[0].fragSize    = 1;

   ccode = NWRequestToAll(6, 1, reqFrag);
#ifdef N_PLAT_OS2
   if(ccode)
      return (ccode);
...
/*
$Header: /SRCS/esmp/usr/src/nw/lib/libnwcalls/rellokst.c,v 1.7 1994/09/26 17:48:59 rebekah Exp $
*/
