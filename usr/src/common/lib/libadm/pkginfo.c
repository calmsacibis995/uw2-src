...
/*LINTLIBRARY*/
#ident	"@(#)libadm:common/lib/libadm/pkginfo.c	1.2.6.10"
#ident  "$Header: $"
...
char *pkginst, *ckvers;
{
	...
	if (strcmp(msg_locale, "C") != 0) {
		(void) sprintf(temp, "%s/%s/install/inst/locale/%s/pkginfo",
					pkgdir, pkginst, msg_locale);
...
char *request, *actual;
{
	if (!request || !actual || (*request == NULL) || (*actual == NULL))
		return (-1);
...
char	*inst, *arch, *vers;
{
	...
	msg_locale = setlocale(LC_MESSAGES, NULL);
	if (strcmp(msg_locale, "C") != 0) {
		(void) sprintf(file, "%s/%s/install/inst/locale/%s/pkginfo",
					pkgdir, inst, msg_locale);
