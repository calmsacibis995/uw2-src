...
#ident	"@(#)libadm:common/lib/libadm/devreserv.c	1.2.10.3"
...
reservlist()
{
	...
	if (lockname = _rsvtabpath()) {
	    ...
	    /* return NULL if lock file does not exist */
	    if ( access(lockname, F_OK) == -1 && errno == ENOENT )
	    {
	    	free(lockname);
		return((struct reservdev **) NULL);
	    }

	    /* Open it */
	    lkfilefd = open(lockname, O_RDONLY);
	    free(lockname);
