...
#ident	"@(#)libsetup:libsetup/table.c	1.2"
...
static void
	...
	if(entry_p == NULL)
	    ...
	    if(DebugLevel > 5) (void) fprintf(stderr, "\tFreed tbl_link.\n");
	    if(entry_p->tbl_freeLink != NULL) linkFree(entry_p->tbl_freeLink);
	    if(DebugLevel > 5) (void) fprintf(stderr, "\tFreed tbl_freeLink.\n");
	    if(entry_p->tbl_string != NULL) free(entry_p->tbl_string);
	    if(DebugLevel > 5) (void) fprintf(stderr, "\tFreed tbl_string.\n");
	    if(entry_p->tbl_free != NULL) entry_p->tbl_free(entry_p->tbl_value);
	    if(DebugLevel > 5) (void) fprintf(stderr, "\tFreed tbl_value.\n");
	    free(entry_p);
	    }
