...
//	int operator== (const Map<S,T>&);
	...
	inline Mapiter<S,T> first() const;
	inline Mapiter<S,T> last() const;
};

template <class S, class T>
inline Mapiter<S,T> Map<S,T>::first() const {
	return ++Mapiter<S,T> (*this);
}

template <class S, class T>
inline Mapiter<S,T> Map<S,T>::last() const {
	return --Mapiter<S,T> (*this);
}
