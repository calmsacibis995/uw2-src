...
const PSETF_ATTLC_P(Graph)	Vertex_graphs_ATTLC = &Vertex::graphs;
const PSETF_ATTLC_P(Edge)	Vertex_edges_ATTLC = &Vertex::edges;
const PSETFG_ATTLC_P(Edge)	Vertex_edges_g_ATTLC = &Vertex::edges_g;
const PSETF_ATTLC_P(Edge)	Vertex_in_edges_ATTLC = &Vertex::in_edges;
const PSETFG_ATTLC_P(Edge)	Vertex_in_edges_g_ATTLC = &Vertex::in_edges_g;
const PSETF_ATTLC_P(Edge)	Vertex_out_edges_ATTLC = &Vertex::out_edges;
const PSETFG_ATTLC_P(Edge)	Vertex_out_edges_g_ATTLC = &Vertex::out_edges_g;
const PSETF_ATTLC_P(Edge)	Vertex_loop_edges_ATTLC = &Vertex::loop_edges;
const PSETFG_ATTLC_P(Edge)	Vertex_loop_edges_g_ATTLC = &Vertex::loop_edges_g;
...
Graph::induced_graph(const Set_of_p<Vertex>& v0_pset)
{
	...
	while (vp = vset_iter.next()) {
		if (contains(vp)) {
			g.insert(vp);
			Set_of_piter<Edge> e1set_iter(vp->in_edges_g(*this));
			Edge* ep;
			while (ep = e1set_iter.next())
				if (v_pset.contains(ep->src()))
					g.insert(ep);
					
			Set_of_piter<Edge> e2set_iter(vp->out_edges_g(*this));
			while (ep = e2set_iter.next())
				if (v_pset.contains(ep->dst()))
					g.insert(ep);
					
			Set_of_piter<Edge> e3set_iter(vp->loop_edges_g(*this));
...
Graph::fill_graph(const Graph& orig_g, Set_of_p<Vertex>& v_pset)
{
	...
	while (vp = vset_iter.next()) {
		if (orig_g.contains(vp)) {
			insert(vp);
			Set_of_piter<Edge> e1set_iter(vp->in_edges_g(orig_g));
			Edge* ep;
			while (ep = e1set_iter.next())
				if (v_pset.contains(ep->src()))
					insert(ep);
			Set_of_piter<Edge> e2set_iter(vp->out_edges_g(orig_g));
			while (ep = e2set_iter.next())
				if (v_pset.contains(ep->dst()))
					insert(ep);
			Set_of_piter<Edge> e3set_iter(vp->loop_edges_g(orig_g));
			while (ep = e3set_iter.next())
				insert(ep);
			v_pset.remove(vp); 	//shrinks set over which to inspect
		}
