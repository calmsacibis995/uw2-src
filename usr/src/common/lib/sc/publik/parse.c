...
// Currently no sorry msgs issued
#if 0
...
// Currently no warning msgs issued
#if 0
...
/* change to non-inline to workaround SVR4/386 +a1 problem */
...
static inline bool interesting(TokenType t)
{
	return (t == FRIEND || 
		t == CLASS ||
		t == TEMPLATE ||
...
static inline void parseAFragment()
{
	switch (TOK) {
	case FRIEND:
		frend();
...
static void cppline()
{
	...
	if (TOK == ID)	// skip "line", if present
	{
		ADV;
	}
	if (TOK == LITINT)
