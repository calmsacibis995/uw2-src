...
Bag_bucketiter_ATTLC<T>::~Bag_bucketiter_ATTLC()
{
	...
	for ( Bag_bucketiter_ATTLC<T>* an_it=my_bag->iter_head ;
...
Bag<T>::insert(const T& value, int count)
{
    ...
    for ( int depth = -1 ; depth < pos.curr_depth ;
...
Bag<T>::remove(const T& value, int count)
{
    ...
    for ( Bag_internal_item_ATTLC* itp = &nodep->item[0] ;
...
Bag<T>::contains(const T& value) const
{
    ...
    for ( int depth = -1 ; depth < pos.curr_depth ; depth++, mask <<= BAG_SHIFT_INCR_ATTLC )
    {
	if ( (pos.curr_value & mask) != (hval & mask) )
	    break;
    }
