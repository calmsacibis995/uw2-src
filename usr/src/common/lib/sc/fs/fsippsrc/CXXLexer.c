...
/*ident	"@(#)sc:fs/fsippsrc/CXXLexer.c	3.1" */
/******************************************************************************
*
...
*/
static inline fastCmp(const String &s, const char *t, int lent)
{
	if ((*(const char*)s != *t) || (s.length() != lent))
		return 0;
	else
		return !memcmp((const char*)s, t, lent);
}
