...
/*ident	"@(#)sc:Time/ctime/time_comm.c	3.3" */
/******************************************************************************
*
...
char tzname_ATTLC[][MAXTZNAME+1] = {"GMT","   "};
...
// If TZ has implementation-defined format (leads with colon),
// then call ctime(3C)'s tzset() to parse it, then move from
// ctime(3C)'s external variables into ours.

    if (*ptr == ':') {
	::tzset();
	timezone_ATTLC = timezone;
	altzone_ATTLC = altzone;
	daylight_ATTLC = daylight;
	strcpy(tzname_ATTLC[0],tzname[0]);
	strcpy(tzname_ATTLC[1],tzname[1]);
	start_dst_ATTLC = end_dst_ATTLC = 0;	// ctime(3C) doesn't give these
	return;
    }
