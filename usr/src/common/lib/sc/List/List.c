...
{ 
	...
	for (register Liztiter_ATTLC* anIt = iter_head; anIt; anIt = s) {
	    s = anIt->nextIt;
	    anIt->nextIt = 0;
	    anIt->theLizt = 0;
	    anIt->pred = 0;
...
{
	delete_all();
        Liztiter_ATTLC *save_head = iter_head; // save the list of iterators
	init_all_to_empty();
        iter_head = save_head;  // restore the list of iterators that was
                                // wiped out by init_all_to_empty()
	reset_all_iters();
}
...
{ 
	for (register Liztiter_ATTLC *anIt = iter_head; anIt; anIt = anIt->nextIt)
	    anIt->reset0();
}
...
Lizt_ATTLC::unget(const Lizt_ATTLC& ll)
{
	    ...
	    if (&ll == this) {
		Lizt_ATTLC l (ll);
        	register lnk_ATTLC*    aLink = l.t;
        	do unget(aLink->copy());
        	while ((aLink = aLink->prv) != l.t);
	    }
	    else {
...
Lizt_ATTLC::put(const Lizt_ATTLC& ll)
{
	    ...
	    if (&ll == this) {
		Lizt_ATTLC l (ll);
        	register lnk_ATTLC*    aLink = l.head();
        	do put(aLink->copy());
        	while (aLink != l.t && (aLink = aLink->nxt, 1));
	    }
	    else {
...
{ 
	if(theLizt && theLizt->iter_head) {
	    if(this==(theLizt->iter_head))
	        ...
	        for (
	            register Liztiter_ATTLC* anIt = theLizt->iter_head; 
	            anIt->nextIt != this;
	            anIt = anIt->nextIt
	        ) ;
	        anIt->nextIt = nextIt;
...
Liztiter_ATTLC::insert_prev(lnk_ATTLC* nl)
{
	if (pred) {
	    ...
	    pred->nxt->prv = nl;
	    pred->nxt = nl;
	    for (register Liztiter_ATTLC *anIt=(theLizt->iter_head); anIt; anIt=anIt->nextIt) {
