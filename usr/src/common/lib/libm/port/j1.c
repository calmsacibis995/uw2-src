...
static const double tpi = 0.6366197723675813430755350535;
...
double x;
{
	...
	if (NANorINF(x) && !INF(x))	/* x is NaN */
		return _domain_err(x,0.0,x,"j1",2);
	if ((y = x) < 0)
		x = -x;
	if (x > 8)
...
double x;
{
	...
	if (NANorINF(x) && !INF(x))	/* x is NaN */
		return _domain_err(x,0.0,x,"y1",2);
	if (x <= 0) {
		double ret;
		...
		return _domain_err(x,0.0,ret,"y1",2);
	}
	if (x > 8)
		return (j1_asympt(x, x, 0));
	z = x * x;
	/* check result didn't underflow */
	if (!z) {
