...
exp_err(float x)
{
	...
	if (FNANorINF(x) && !FINF(x))		/* NaN */
		return _float_domain(x,0.0F,x,"expf",4);
	else if ( x < 0 ){			/* underflow */
		exc.type = UNDERFLOW;
		ret = 0;
	} else {				/* overflow */
		exc.type = OVERFLOW;
		ret = (_lib_version == c_issue_4 ? MAXFLOAT :
			(float)HUGE_VAL);
	}
