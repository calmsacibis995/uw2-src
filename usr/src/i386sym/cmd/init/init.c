...
#ident	"@(#)init:i386sym/cmd/init/init.c	1.9"
...
debug("%d:initialize():ENTER\n",getpid());
	 ...
	 * whether or not TP is configured in the system. If we are non-MAC
	 * allow the user to 'de-config' TP
	 */
	...
	if ((pid=fork()) != FAILURE) {
		if (pid==0) {
			...
			if ((auditctl(ASTATUS, &actl, sizeof(actl_t)))==FAILURE) {
		                console(MM_ERROR,
				    ":44:auditctl: unable to get status of auditing\n");
				exit(1);
			}
